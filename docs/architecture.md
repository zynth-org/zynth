...
This document covers how the Rune framework works internally. It is intended to aid in understanding the code, understanding how Rune achieves high performance on mobile devices using SolidJS, and enabling contributors to modify the core system.

Note that Rune is an experimental framework that makes different design decisions than React Native or other hybrid frameworks. These decisions prioritize performance and "native feel" over backward compatibility or web-standard strictness.
...
*   [Notes about the Runtime (RuneKit)](#notes-about-the-runtime-runekit)
...
*   **Fine-Grained Reactivity**
    ...
    Rune uses [SolidJS](https://www.solidjs.com/) as its core engine. Unlike React Native, there is no Virtual DOM (VDOM). Updates are surgical: when a signal changes, only the specific property on the specific native node is updated. This eliminates the "render phase" entirely.
...
*   **Platform Agnostic Core**
    ...
    The core renderer (`@rune/core`) is completely decoupled from the OS. It speaks a generic protocol ("create node", "set prop"). This allows the same application code to drive iOS, Android, and Web backends (via a DOM adapter) without changing the business logic.
...
The Rune pipeline can be visualized as a direct pipeline from a Reactive Signal to a Native View mutation.
...
Rune implements the SolidJS `Universal Renderer` API. Instead of creating HTML elements, it creates lightweight JavaScript objects (HostNodes) that represent a reference to a native view.
...
When a signal updates, Solid calls the renderer's `setProperty` or `insertNode` methods. These methods push operations into a queue managed by the Rune Host (`src/host/ios.ts` or `src/host/android.ts`).

## Notes about Rendering

### SolidJS Universal Renderer

The file `packages/rune-core/src/renderer.ts` creates the Solid renderer. It maps standard DOM operations to our Host API:
...
To maximize performance, we do not cross the JSI boundary for every single property update. Instead, `rune-core` implements an operation queue.
...
Rune implements a custom view recycling system in JavaScript (similar to `RecyclerView` or `UITableView`) for its `FlatList` component.

*   **The Problem**: Creating native views is expensive. Destroying and recreating them during scrolling causes frame drops.
*   **The Solution**: When a node is "removed" from the list, Rune doesn't destroy the native view. It keeps the `HostNode` ID in a pool. When a new row is needed, Rune reclaims an ID from the pool and updates its properties. This avoids the expensive `createNode` and layout initialization steps on the native side.

## Notes about the Runtime (RuneKit)

The native runtimes (`packages/rune-ios` and `packages/rune-android`) are collectively known as **RuneKit**.
...
*   **iOS**: Implemented in `RuneUIManager.mm`. It maps integer IDs to `UIView` instances / Yoga Nodes.
*   **Android**: Implemented in `RuneBridge.kt` (via JNI). It maps IDs to `Android.View` instances / Yoga Nodes.

### Yoga Layout

Rune uses [Yoga](https://yogalayout.dev/) (the same layout engine as React Native) to implement Flexbox.
...
Rune ensures that heavy JS work does not block the UI thread, except when synchronous layout measurement is explicitly requested.

## Notes about Navigation

### Memory-based Routing

`@rune/memory-router` is a pure JavaScript implementation of a navigation stack. It maintains the state of routes, parameters, and history.

We chose a memory router over a native-controller-based router (like `react-native-screens`' native stack) to support the **Hypervisor** architecture. In a Hypervisor setup, multiple independent Rune apps might run on the same screen. If they all tried to control the single global `UINavigationController`, chaos would ensue. A memory router keeps navigation state isolated per-app.

### Native Screen Integration

While the *state* is in memory, the *visuals* use native primitives.
*   `@rune/screens` exposes `Screen` and `ScreenContainer`.
...
`@rune/hypervisor` allows a "Host" Rune app to embed "Guest" Rune apps.
*   **Isolation**: Each Guest runs in its own `RuneRuntime` (separate Hermes VM).
...
Animations in Rune are physics-based and interruptible. To avoid the latency of the JS bridge during 60fps animations, we use **Shared Values**.
...
We use [Rsbuild](https://rsbuild.dev/) (based on Rspack) for blazing fast builds. The `@rune/rsbuild-plugin` manages the complexity of targeting two very different environments:
...
Skyhook is the backend service that powers Rune's "Prompt-to-App" capability.
...
3.  **Build**: It runs `rune bundle` inside the container.
