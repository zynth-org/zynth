...
### Phase 3: Shadows (Medium Priority, Completed)
...
- **Delivered:**
  - Shared `RuneShadowParser` implemented on Android and iOS; it normalizes CSS `box-shadow` strings and RN-style shadow props into a `ShadowLayer` model (offset, blur, spread, color, inset).
  - Added `boxShadow`, `shadowColor`, `shadowOffset`, `shadowOpacity`, `shadowRadius`, and `elevation` to `Style.kt` and `HostTypes.ts` so the parser and native bridges receive normalized shadow data.
  - Android:
    - Implemented `RuneShadowParser.kt` and integrated parsing/merge logic into `Style.fromJson`.
    - `RunePropApplier` applies `elevation` via `ViewCompat.setElevation` and uses `ViewOutlineProvider.BACKGROUND` to respect rounded corners; on Android P+ we set `outlineAmbientShadowColor`/`outlineSpotShadowColor` when a primary shadow is provided.
    - For pre-API 28 devices and where a full CSS shadow can't be represented, the implementation falls back to elevation-only behavior.
  - iOS:
    - Implemented `RuneShadowParser.h/.m` and integrated parsing in `SNUIManager`.
    - `SNApplyShadowStyleToView` consumes the normalized `ShadowLayer` model and applies `layer.shadowColor`, `shadowOffset`, `shadowOpacity`, `shadowRadius`, and `shadowPath` (from mask or corner radius). For `elevation` without specific `boxShadow` values, we synthesize a suitable shadow.
  - Dev harness & examples:
    - Added example stories in `apps/components/src/components/styles/ShadowExample.tsx` demonstrating RN-style shadow props, `boxShadow` CSS strings, and `elevation` as a fallback.
  - Notes / Limitations:
    - The approach currently favors the primary shadow (first in list) for rendering and uses platform fallbacks where needed (elevation on older Android APIs, single-layer shadows on iOS), while preserving the normalized model for future enhancements supporting multiple shadows and spread/inset emulation.
