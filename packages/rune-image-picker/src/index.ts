...
export const ImagePicker = {
  getCameraPermissionsAsync: async (): Promise<PermissionResponse> => {
    ...
    if (!bridge || !bridge.call) return { status: 'undetermined', granted: false, canAskAgain: true };
    return (await bridge.call("ImagePicker", "getCameraPermissionsAsync")) as PermissionResponse;
  },
  ...
  requestCameraPermissionsAsync: async (): Promise<PermissionResponse> => {
    ...
    return new Promise((resolve, reject) => {
        ...
        try {
          bridge.call("ImagePicker", "requestCameraPermissionsAsync", { requestId });
        } catch (e) {
          subscription.remove();
          reject(e);
        }
  ...
  launchCameraAsync: async (
    ...
    if (!bridge || !bridge.call) {
      console.warn(
        "[Rune] Native modules bridge not available. ImagePicker skipped."
      );
    ...
    return new Promise((resolve, reject) => {
      ...
      console.log(`[ImagePicker] Listening for result with requestId: ${requestId}`);
      const subscription = sharedNativeEventEmitter.addListener(
        "ImagePicker.result",
        (payload: any) => {
          console.log(`[ImagePicker] Received event:`, JSON.stringify(payload));
          if (payload && payload.requestId === requestId) {
            console.log(`[ImagePicker] Match found for requestId: ${requestId}`);
            subscription.remove();
            ...
            if (payload.error) {
             ...
             console.log(`[ImagePicker] Ignored event (requestId mismatch):`, payload?.requestId);
          }
        }
      );
      ...
      try {
        ...
        bridge.call("ImagePicker", "launchCameraAsync", { options: options || {}, requestId });
      } catch (e) {
  ...
  launchImageLibraryAsync: async (
    ...
    return new Promise((resolve, reject) => {
      console.log(`[ImagePicker] Listening for library result with requestId: ${requestId}`);
      const subscription = sharedNativeEventEmitter.addListener(
        "ImagePicker.result",
        (payload: any) => {
          console.log(`[ImagePicker] Received event:`, JSON.stringify(payload));
      ...
      try {
        ...
        bridge.call("ImagePicker", "launchImageLibraryAsync", { options: options || {}, requestId });
      } catch (e) {
        console.error(`[ImagePicker] Native call failed:`, e);
        subscription.remove();
        reject(e);
