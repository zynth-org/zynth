...
This template provides a complete starting point for creating a native module that lives inside a Rune app (under `apps/**/modules`).
...
```bash
cp -r packages/rune-templates/app-module apps/my-app/modules/my-module
...
```
app-module/
├── package.json              # Local module metadata with runeNative config
