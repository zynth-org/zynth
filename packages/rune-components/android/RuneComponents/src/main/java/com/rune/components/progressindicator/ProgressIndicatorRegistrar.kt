    ...
    registry.register(
      RuneComponentDescriptor(
        ...
        onNodeCreated = { manager, node ->
          val indicator = node.view as? RuneProgressIndicatorView ?: return@RuneComponentDescriptor
        ...
        applyProperty = { node, name, value ->
          (node.view as? RuneProgressIndicatorView)?.let { indicator ->
            try {
              when (name) {
                "color" -> {
