    ...
    registry.register(
      RuneComponentDescriptor(
        ...
        onNodeCreated = { manager: RuneUIManager, node: RuneUIManager.Node ->
          val switchView = node.view as? RuneSwitchView ?: return@RuneComponentDescriptor
          switchView.nodeId = node.id
          
          // Set up the value change listener
          switchView.listener = object : RuneSwitchView.Listener {
        ...
        applyProperty = { node, name, value ->
          (node.view as? RuneSwitchView)?.let { switchView ->
            try {
              when (name) {
                "value" -> {
