package com.rune.components.slider
    ...
    registry.register(
      RuneComponentDescriptor(
        ...
        onNodeCreated = { manager: RuneUIManager, node: RuneUIManager.Node ->
          val sliderView = node.view as? RuneSliderView ?: return@RuneComponentDescriptor
          sliderView.nodeId = node.id

          sliderView.listener = object : RuneSliderView.Listener {
        ...
        applyProperty = { node, name, value ->
          (node.view as? RuneSliderView)?.let { sliderView ->
            try {
              when (name) {
                "value" -> {
