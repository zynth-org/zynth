...
@objcMembers
public final class RuneModalView: UIView {
  private weak var manager: SNUIManager?
  private weak var node: SNNode?

  private let contentHost: UIView = {
    let view = RuneModalPassthroughView()
    view.isHidden = false
    view.backgroundColor = .clear
    return view
  }()

  private lazy var presenter = RuneModalPresenter(contentHost: contentHost, host: self)
