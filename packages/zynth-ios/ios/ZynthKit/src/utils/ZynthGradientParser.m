...
static NSArray<NSString *> *SNZynthSplitGradientArgs(NSString *raw) {
...
static CGFloat SNZynthNormalizeAngle(CGFloat angle) {
  CGFloat a = fmod(angle, 360.0);
  if (a < 0) a += 360.0;
  return a;
}

static CGFloat SNZynthParseDirection(NSString *dir, BOOL *ok) {
...
static CGFloat SNZynthParseAngleToken(NSString *token, BOOL *ok) {
  ...
  if ([trimmed hasPrefix:@"to "]) {
    BOOL dirOk = NO;
    CGFloat angle = SNZynthParseDirection([trimmed substringFromIndex:3], &dirOk);
    *ok = dirOk;
    return angle;
  }
...
static NSNumber *_Nullable SNZynthParsePosition(NSString *raw) {
...
static ZynthGradientStop *_Nullable SNZynthParseStop(NSString *token) {
  NSString *trimmed = [token stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
  ...
  UIColor *color = [ZynthColorParser parseColor:[colorPart stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]]];
  if (!color) return nil;
  NSNumber *pos = positionPart ? SNZynthParsePosition(positionPart) : nil;
  return [[ZynthGradientStop alloc] initWithColor:color position:pos];
}

@implementation ZynthGradientStop
...
@implementation ZynthLinearGradient
...
@implementation ZynthGradientParser
...
+ (nullable ZynthLinearGradient *)parse:(id)value {
  ...
  NSArray<NSString *> *tokens = SNZynthSplitGradientArgs(inside);
  if (tokens.count < 2) return nil;

  CGFloat angle = 180.f;
  NSUInteger startIndex = 0;
  BOOL angleOk = NO;
  angle = SNZynthParseAngleToken(tokens.firstObject, &angleOk);
  if (angleOk) {
    startIndex = 1;
  } else {
    angle = 180.f;
  ...
  NSMutableArray<ZynthGradientStop *> *stops = [NSMutableArray array];
  for (NSUInteger i = startIndex; i < tokens.count; i++) {
    ZynthGradientStop *stop = SNZynthParseStop(tokens[i]);
    if (stop) [stops addObject:stop];
  }
  if (stops.count < 2) return nil;

  return [[ZynthLinearGradient alloc] initWithAngle:SNZynthNormalizeAngle(angle) stops:stops];
}
