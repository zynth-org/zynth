...
extern void SNApplyGradientToView(UIView *view, ZynthLinearGradient *gradient);
...
static NSTimeInterval const kZynthLayoutDefaultDuration = 0.3;

static NSString *ZynthLayoutTransitionString(id value) {
  return [value isKindOfClass:[NSString class]] ? (NSString *)value : nil;
}

static NSTimeInterval ZynthLayoutTransitionDuration(NSDictionary *config) {
  id value = config[@"duration"];
  if ([value isKindOfClass:[NSNumber class]]) {
    return MAX(0.0, [(NSNumber *)value doubleValue] / 1000.0);
  }
  return kZynthLayoutDefaultDuration;
}

static NSTimeInterval ZynthLayoutTransitionDelay(NSDictionary *config) {
...
static UIViewAnimationOptions ZynthLayoutTransitionOptions(NSDictionary *config) {
  NSString *easing = ZynthLayoutTransitionString(config[@"easing"]);
...
static BOOL ZynthLayoutTransitionIsLinear(NSDictionary *config) {
  NSString *type = ZynthLayoutTransitionString(config[@"type"]) ?: @"linear";
  return [type isEqualToString:@"linear"];
}

static BOOL ZynthOriginTokenIsHorizontal(NSString *token) {
  return [token isEqualToString:@"left"] || [token isEqualToString:@"right"];
}

static BOOL ZynthOriginTokenIsVertical(NSString *token) {
  return [token isEqualToString:@"top"] || [token isEqualToString:@"bottom"];
}

static BOOL ZynthParseOriginToken(NSString *token, BOOL isX, CGFloat *outValue, BOOL *outIsPercent) {
  if (!token) return NO;
...
static CGPoint ZynthResolveTransformOriginPoint(id origin, CGSize size) {
  ...
  if (originArray.count > 0) {
    ...
    if (tokens.count == 1) {
      ...
      if (ZynthOriginTokenIsVertical(token)) {
        xToken = @"center";
        yToken = token;
      } else {
        xToken = token;
      ...
      if (ZynthOriginTokenIsVertical(first) && ZynthOriginTokenIsHorizontal(second)) {
        xToken = second;
        yToken = first;
      } else {
        xToken = first;
  ...
  if (xRaw) {
    if ([xRaw isKindOfClass:[NSNumber class]]) {
      ...
      ZynthParseOriginToken((NSString *)xRaw, YES, &xValue, &xIsPercent);
    }
  } else if (xToken) {
    ZynthParseOriginToken(xToken, YES, &xValue, &xIsPercent);
  }
  ...
  if (yRaw) {
    if ([yRaw isKindOfClass:[NSNumber class]]) {
      ...
      ZynthParseOriginToken((NSString *)yRaw, NO, &yValue, &yIsPercent);
    }
  } else if (yToken) {
    ZynthParseOriginToken(yToken, NO, &yValue, &yIsPercent);
  }
...
static void ZynthApplyTransformOrigin(UIView *view, id origin) {
  CGSize size = view.bounds.size;
  if (size.width <= 0 || size.height <= 0) return;
  CGPoint point = ZynthResolveTransformOriginPoint(origin, size);
...
@implementation SNUIManager (ZynthLayout)

- (void)zynth_startDisplayLinkIfNeeded {
  if (self.displayLink) return;
  dispatch_async(dispatch_get_main_queue(), ^{
    if (self.displayLink) return;
    [self zynth_setDisplayLink:[CADisplayLink displayLinkWithTarget:self selector:@selector(zynth_displayLinkTick:)]];
...
- (void)zynth_stopDisplayLink {
  if (self.displayLink) {
    ...
    [self zynth_setDisplayLink:nil];
  }
}

- (void)zynth_markNeedsFlush {
  dispatch_async(dispatch_get_main_queue(), ^{
    [self zynth_setNeedsFlush:YES];
...
- (void)zynth_performFlush {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    NSArray<NSNumber *> *surfaceIds = [self zynth_allSurfaceIds];
    ...
    for (NSNumber *sid in surfaceIds) {
      ...
      YGNodeRef rootYoga = [self zynth_rootYogaForSurface:surfaceId];
      ...
      if ((boundsSize.width <= 0 || boundsSize.height <= 0) && surfaceId == [self zynth_rootSurfaceId]) {
    ...
    [self.nodes enumerateKeysAndObjectsUsingBlock:^(NSNumber *key, SNNode *obj, BOOL *stop) {
      ...
      @try {
        ...
        ZynthApplyTransformOrigin(obj.view, obj.transformOrigin);

        NSDictionary *layoutTransition = obj.layoutTransition;
        if (layoutTransition && ZynthLayoutTransitionIsLinear(layoutTransition)) {
          ...
          if (previousValue) {
            ...
            if (prevWidth > 0.f && prevHeight > 0.f && nextWidth > 0.f && nextHeight > 0.f) {
              ...
              if (fabs(deltaX) > 0.5 || fabs(deltaY) > 0.5 || fabs(scaleX - 1.0) > 0.01 || fabs(scaleY - 1.0) > 0.01) {
                ...
                NSTimeInterval duration = ZynthLayoutTransitionDuration(layoutTransition);
                NSTimeInterval delay = ZynthLayoutTransitionDelay(layoutTransition);
                UIViewAnimationOptions options = ZynthLayoutTransitionOptions(layoutTransition);
        ...
        if (frameChanged) {
          ...
          if (obj.latestStyle) {
            ...
            ZynthLinearGradient *gradient = [ZynthGradientParser parse:bgValue];
            if (!gradient && [bgValue isKindOfClass:[NSString class]]) {
              gradient = [ZynthGradientParser parse:bgValue];
            }
            if (gradient) {
    ...
    for (NSNumber *sid in surfaceIds) {
      [self zynth_dispatchSurfaceFirstFrameIfNeeded:sid.intValue];
    }
  });
}
...
- (void)zynth_dispatchLayoutEventForNode:(SNNode *)node force:(BOOL)force {
  ...
  [self zynth_dispatchEvent:@"onLayout" payload:payload toNode:node];
}
