...
#import "SNHexColor.h"
#import "utils/ZynthTransformParser.h"
#import "utils/ZynthGradientParser.h"
#import "utils/ZynthShadowParser.h"
#import <Yoga/Yoga.h>
#import <QuartzCore/QuartzCore.h>
#import <objc/message.h>
#import <stdatomic.h>

@protocol ZynthPlaceholderTone <NSObject>
- (void)applyPlaceholderToneFromTextColor;
@end

static NSString *const kZynthBorderLayerName = @"zynth-border-style";
static NSString *const kZynthGradientLayerName = @"zynth-background-gradient";
static const int kZynthSurfaceIdBase = 1 << 20;
static const double kZynthBatchTimeBudgetMs = 4.0;
static const NSUInteger kZynthBatchMaxOps = 200;
...
static _Atomic int sNextGuestSurfaceId = kZynthSurfaceIdBase;
...
@property(nonatomic, strong) ZynthComponentRegistry *componentRegistry;
...
- (CADisplayLink *)zynth_displayLink { return _displayLink; }
- (void)zynth_setDisplayLink:(CADisplayLink *_Nullable)displayLink { _displayLink = displayLink; }
- (BOOL)zynth_needsFlush { return _needsFlush; }
- (void)zynth_setNeedsFlush:(BOOL)needsFlush { _needsFlush = needsFlush; }
- (NSMutableDictionary<NSString *, NSDictionary *> *)zynth_eventPayloads { return _eventPayloads; }
- (void)zynth_setEventPayloads:(NSMutableDictionary<NSString *, NSDictionary *> *)payloads { _eventPayloads = payloads; }

- (void)dealloc {
  [self zynth_stopDisplayLink];
...
- (instancetype)initWithRootView:(UIView *)rootView isGuest:(BOOL)isGuest {
  if (self = [super init]) {
    _componentRegistry = [ZynthComponentRegistry shared];
    _root = rootView;
    ...
    _surfaceIdSeed = kZynthSurfaceIdBase;
...
- (int)zynth_rootSurfaceId { return _primarySurfaceId; }
...
- (BOOL)zynth_hasSurface:(int)surfaceId {
  return [self zynth_rootViewForSurface:surfaceId] != nil;
}

- (BOOL)zynth_isSurfaceRootId:(NSNumber *)nodeId {
  if (!nodeId) return NO;
  int sid = nodeId.intValue;
  return sid == self.rootSurfaceId || self.surfaceRoots[@(sid)] != nil;
}

- (int)zynth_allocateSurfaceId {
...
- (NSNumber *)registerSurfaceWithRootView:(UIView *)rootView surfaceId:(NSNumber *_Nullable)surfaceId {
  ...
  if (surfaceId != nil) {
    ...
    int candidate = [self zynth_allocateSurfaceId];
    int minSafe = self.nextId + kZynthSurfaceIdBase;
    if (candidate < minSafe) {
      candidate = minSafe;
    }
    while ([self zynth_hasSurface:candidate]) {
      candidate += kZynthSurfaceIdBase;
    }
    sid = candidate;
  }
  if ([self zynth_hasSurface:sid]) {
    return @(sid);
  }
  if (self.nextId <= sid) {
    self.nextId = sid + 1;
...
- (void)unregisterSurface:(int)surfaceId {
  ...
  for (NSNumber *key in allKeys) {
    ...
    ZynthComponentDescriptor *descriptor = [self.componentRegistry getDescriptor:node.type];
...
- (void)setActiveSurface:(int)surfaceId {
  if (![self zynth_hasSurface:surfaceId]) {
...
- (void)zynth_dispatchSurfaceFirstFrameIfNeeded:(int)surfaceId {
  if (![NSThread isMainThread]) {
    dispatch_async(dispatch_get_main_queue(), ^{
      [self zynth_dispatchSurfaceFirstFrameIfNeeded:surfaceId];
    });
    return;
  }
  NSNumber *key = @(surfaceId);
  if ([self.surfaceFirstFrameDispatched containsObject:key]) return;
  UIView *rootView = [self zynth_rootViewForSurface:surfaceId];
...
- (NSNumber *)createNode:(NSString *)type {
  ...
  ZynthComponentDescriptor *componentDescriptor = [self.componentRegistry getDescriptor:type];
  ...
  if (!v) {
    ...
    NSLog(@"[ZynthKit] WARNING: No descriptor found for type '%@', using fallback UIView", type);
    v = [[UIView alloc] init];
  }
...
static void SNRemoveCustomBorderLayers(UIView *view) {
  ...
  for (CALayer *layer in sublayers) {
    if ([layer.name isEqualToString:kZynthBorderLayerName]) {
      [layer removeFromSuperlayer];
    }
    if ([layer.name isEqualToString:kZynthGradientLayerName]) {
      [layer removeFromSuperlayer];
    }
  }
}

static NSArray<NSNumber *> *SNResolveGradientLocations(NSArray<ZynthGradientStop *> *stops) {
...
void SNApplyGradientToView(UIView *view, ZynthLinearGradient *gradient) {
  if (!view) return;
  ...
  layer.name = kZynthGradientLayerName;
...
static void SNApplyShadowStyleToView(UIView *view, NSArray<ZynthShadowLayer *> *layers, NSNumber *elevation) {
  if (!view) return;

  NSMutableArray<ZynthShadowLayer *> *effectiveLayers = [layers mutableCopy];
  ...
  if (effectiveLayers.count == 0 && elevation) {
    ...
    ZynthShadowLayer *synthetic = [[ZynthShadowLayer alloc] initWithOffsetX:0 offsetY:e / 2.f blurRadius:e spread:0 color:color inset:NO];
  ...
  NSArray<CALayer *> *sublayers = [view.layer.sublayers copy];
  for (CALayer *layer in sublayers) {
    if ([layer.name isEqualToString:@"zynth-shadow-layer"]) {
      [layer removeFromSuperlayer];
    }
  }

  ZynthShadowLayer *primary = effectiveLayers.firstObject;
  if (primary) {
...
static void SNApplyBorderStyleToView(UIView *view, NSDictionary *style, BOOL overflowClip) {
  SNRemoveCustomBorderLayers(view); // Removes all layers named kZynthBorderLayerName
...
- (void)sn_applyStyleDictionary:(NSDictionary *)style toNode:(SNNode *)n {
  ...
  if (viewStyleDirty) {
    ...
    ZynthLinearGradient *gradient = [ZynthGradientParser parse:backgroundValue];
    ...
    if (transform) {
      n.view.layer.transform = [ZynthTransformParser parse:transform];
    } else {
    ...
    SEL updateCorner = NSSelectorFromString(@"zynth_updateConfigurationCornerRadiusIfNeeded");
    ...
    NSArray<ZynthShadowLayer *> *cssShadow = [ZynthShadowParser parse:style[@"boxShadow"]];
    NSArray<ZynthShadowLayer *> *rnShadow = [ZynthShadowParser fromReactNativeColor:shadowColor
                                                                          opacity:shadowOpacity
                                                                           radius:shadowRadius
                                                                          offsetX:shadowOffsetX
                                                                          offsetY:shadowOffsetY];
    NSArray<ZynthShadowLayer *> *shadows = [ZynthShadowParser merged:cssShadow fallback:rnShadow];
    ...
    if ([n.view respondsToSelector:@selector(applyPlaceholderToneFromTextColor)]) {
      ...
      if (color && [n.view respondsToSelector:@selector(setTextColor:)]) {
        ...
        [(id<ZynthPlaceholderTone>)n.view applyPlaceholderToneFromTextColor];
      }
...
- (void)setProp:(NSNumber *)nodeId name:(NSString *)name valueJSON:(NSString *)json {
  ...
  ZynthComponentDescriptor *componentDescriptor = [self.componentRegistry getDescriptor:n.type];
  ...
  if ([name isEqualToString:@"pointerEvents"]) {
    ...
    [self zynth_updateInteractionStateForNode:n];
...
- (void)setHandler:(NSNumber *)nodeId name:(NSString *)name {
  ...
  if ([name isEqualToString:@"onPress"]) {
    [self zynth_attachTapRecognizerForNode:n];
    return;
  }
  ...
  if ([name isEqualToString:@"onLayout"]) {
    ...
    [self zynth_dispatchLayoutEventForNode:n force:YES];
    return;
  }
}
...
- (void)applyBatch:(NSString *)batchJSON {
  ...
  applyOperations = ^(NSArray *operations, NSUInteger startIndex) {
    ...
    for (; end < count; end++) {
      if ((end - startIndex) >= kZynthBatchMaxOps) {
        break;
      }
      double elapsedMs = (CFAbsoluteTimeGetCurrent() - startTime) * 1000.0;
      if (elapsedMs >= kZynthBatchTimeBudgetMs) {
        break;
      }
      NSUInteger i = end;
...
- (void)insertChild:(NSNumber *)parentId child:(NSNumber *)childId index:(NSNumber *)index {
  ...
  if ([self zynth_isSurfaceRootId:parentId]) {
    UIView *rootView = [self zynth_rootViewForSurface:parentId.intValue];
    YGNodeRef rootYoga = [self zynth_rootYogaForSurface:parentId.intValue];
    ...
    ZynthComponentDescriptor *parentDescriptor = [self.componentRegistry getDescriptor:p.type];
    ...
    if (isScrollViewParent) {
      typedef void (*ZynthScrollInsertIMP)(id, SEL, UIView *, NSInteger);
      ZynthScrollInsertIMP insertIMP = (ZynthScrollInsertIMP)objc_msgSend;
...
- (void)removeChild:(NSNumber *)parentId child:(NSNumber *)childId {
  ...
  ZynthComponentDescriptor *componentDescriptor = [self.componentRegistry getDescriptor:c.type];
  ...
  if ([self zynth_isSurfaceRootId:parentId]) {
    ...
    if (isScrollViewParent) {
      typedef void (*ZynthScrollRemoveIMP)(id, SEL, UIView *);
      ZynthScrollRemoveIMP removeIMP = (ZynthScrollRemoveIMP)objc_msgSend;
...
- (void)clearAllNodes {
  ...
  self.surfaceIdSeed = kZynthSurfaceIdBase;
  self.activeSurfaceId = self.rootSurfaceId;
  self.nextId = 1;
}
...
- (void)sn_startDisplayLinkIfNeeded {
  [self zynth_startDisplayLinkIfNeeded];
}
