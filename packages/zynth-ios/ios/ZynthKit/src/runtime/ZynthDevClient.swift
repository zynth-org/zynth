  ...
  final class ZynthDevClient: NSObject, URLSessionWebSocketDelegate {
    ...
    private func openSocket() {
      guard let socketURL = makeWebSocketURL(from: baseURL) else {
        print("[ZynthDevClient] ❌ Invalid dev server URL: \(baseURL.absoluteString)")
        return
      }

      print("[ZynthDevClient] 🔌 Opening WebSocket connection to: \(socketURL.absoluteString)")
    ...
    private func receiveNextMessage() {
      ...
      socket.receive { [weak self] result in
        ...
        case .failure(let error):
          print("[ZynthDevClient] receive error: \(error.localizedDescription)")
    ...
    private func handleTextMessage(_ text: String) {
      ...
      print("[ZynthDevClient] Received message type: \(type)")

      var shouldForward = true
      switch type {
      case "hash":
        if let hash = payload["data"] as? String {
          print("[ZynthDevClient] 🔑 New build hash: \(hash)")
        }

      case "ok", "still-ok":
        print("[ZynthDevClient] ✅ Compilation OK")
      // Forward to runtime so it can decide whether to apply updates

      case "warnings":
        if let warnings = payload["data"] as? [[String: Any]] {
          print("[ZynthDevClient] ⚠️ Compilation warnings (\(warnings.count))")
      ...
      case "errors":
        if let errors = payload["data"] as? [[String: Any]] {
          print("[ZynthDevClient] ❌ Compilation errors (\(errors.count))")
          for error in errors {
      ...
      default:
        ...
        print("[ZynthDevClient] Forwarding message type '\(type)' to runtime")
      }
    ...
    private func sendHello() {
      ...
      {
        print("[ZynthDevClient] 👋 Sending hello message")
        socket.send(.string(json)) { error in
          if let error {
            print("[ZynthDevClient] ❌ Failed to send hello: \(error.localizedDescription)")
          } else {
            print("[ZynthDevClient] ✅ Hello message sent")
          }
        }
      }
    }
    ...
    private func sendPing() {
      ...
      socket.sendPing { error in
        if let error {
          print("[ZynthDevClient] ping failed: \(error.localizedDescription)")
          self.scheduleReconnect()
        }
      }
    }
    ...
    func urlSession(
      ...
      queue.async { [weak self] in
        ...
        print("[ZynthDevClient] ✅ WebSocket connected to \(urlString)")
        self.sendHello()
        self.receiveNextMessage()
      }
    }
    ...
    func urlSession(
      ...
      queue.async { [weak self] in
        ...
        print(
          "[ZynthDevClient] ⚠️ WebSocket closed (code: \(closeCode.rawValue), reason: \(reasonString))"
        )
        self?.scheduleReconnect()
      }
    }
