...
//
  ...
  extension ZynthRuntime {
    ...
    var devClient: ZynthDevClient? {
      get {
        objc_getAssociatedObject(self, &AssociatedKeys.devClient) as? ZynthDevClient
      }
      set {
    ...
    var lastDevBundle: ZynthDevBundle? {
      get {
        objc_getAssociatedObject(self, &AssociatedKeys.lastDevBundle) as? ZynthDevBundle
      }
      set {
        objc_setAssociatedObject(
    ...
    var statusBar: ZynthDevStatusBar {
      if let existing = objc_getAssociatedObject(self, &AssociatedKeys.statusBar)
        as? ZynthDevStatusBar
      {
        return existing
      }
      let newStatusBar = ZynthDevStatusBar()
      objc_setAssociatedObject(
  ...
  extension ZynthRuntime {
    ...
    func configureDevServer() {
      print("[ZynthRuntime] 🔧 Configuring dev server support...")
        ...
        code:
          ...
          if (typeof globalThis.__zynth_receiveHMRMessage !== 'function') {
            globalThis.__zynth_receiveHMRMessage = function(payload) {
              console.log('[Zynth HMR] Received message:', typeof payload);
              try {
              ...
              if (payload && typeof globalThis.__zynth_refresh === 'function') {
                console.log('[Zynth HMR] Calling __zynth_refresh with type:', payload.type);
                globalThis.__zynth_refresh(payload);
              } else if (payload && typeof globalThis.__zynth_requestFullReload === 'function') {
                console.log('[Zynth HMR] Calling __zynth_requestFullReload');
          ...
          if (typeof globalThis.__zynth_refresh !== 'function') {
            globalThis.__zynth_refresh = function(payload) {
              console.warn('[Zynth HMR1] Refresh invoked with no runtime listener', payload && payload.type);
            };
          ...
          console.log('[Zynth HMR] Runtime hooks installed');
          """
      )

      // Auto-connect to dev server if ZYNTH_DEV_SERVER_URL is set
      let env = ProcessInfo.processInfo.environment
      if let runtimeOverride = env["ZYNTH_DEV_RUNTIME"]?.trimmingCharacters(
      ...
      var urlString = env["ZYNTH_DEV_SERVER_URL"]
      if urlString == nil {
        urlString = Bundle.main.object(forInfoDictionaryKey: "ZynthDevServerURL") as? String
      }

      if let urlString, let rawURL = URL(string: urlString) {
        let url = normalizeDevServerURL(rawURL)
        devServerURL = url
        let tokenEnv = sanitizedToken(env["ZYNTH_DEV_SERVER_TOKEN"])
        devServerToken = tokenEnv
        installDevServerGlobal(url: url, token: tokenEnv)
        print("[ZynthRuntime] 🌐 Connecting to dev server at \(url.absoluteString)")
        connectToDevServer(url, token: tokenEnv)
      } else {
        print("[ZynthRuntime] ℹ️ ZYNTH_DEV_SERVER_URL not set, skipping auto-connect")
      }
    }
  ...
  extension ZynthRuntime {
    ...
    public func connectToDevServer(_ url: URL, token: String? = nil) {
      ...
      if devClient == nil {
        devClient = ZynthDevClient(url: normalizedURL, runtime: self, token: cleanedToken)
      } else {
    ...
    func handleDevMessage(_ text: String) {
      guard
        ...
        print("[ZynthRuntime] HMR message is not valid JSON, ignoring")
        return
      }

      print("[ZynthRuntime] 📨 Processing HMR message type: \(type)")
      ...
      case "hash":
        ...
        if let hash = payload["data"] as? String {
          print("[ZynthRuntime] 🔑 Build hash updated: \(hash)")
          latestDevHash = hash
        }
      ...
      default:
        ...
        _ = runtime.callGlobal("__zynth_receiveHMRMessage", args: [text])
      }
    }

    private func handleUpdateMessage(_ payload: [String: Any]) {
      print("[ZynthRuntime] 🔥 Hot update message received")
      ...
      if let data = payload["data"] as? [String: Any] {
        print("[ZynthRuntime] Update data: \(data.keys)")
      }
    ...
    private func handleCompilationSuccess(trigger: String) {
      if isApplyingHotUpdate {
        print("[ZynthRuntime] ⏳ Ignoring \(trigger) message; hot update is already in flight")
        return
      }

      guard devServerURL != nil else {
        print("[ZynthRuntime] ℹ️ Received \(trigger) without dev server URL; skipping")
        return
      }

      guard lastDevBundle != nil else {
        print("[ZynthRuntime] ℹ️ Received \(trigger) before initial dev bundle loaded; waiting")
        return
      }

      guard let latestHash = latestDevHash else {
        print("[ZynthRuntime] ℹ️ \(trigger) message arrived without a hash; performing full reload")
      ...
      if lastAppliedDevHash == nil {
        print("[ZynthRuntime] ℹ️ Recording initial hash \(latestHash); skipping hot update trigger")
        lastAppliedDevHash = latestHash
        return
      }

      if let appliedHash = lastAppliedDevHash, appliedHash == latestHash {
        print("[ZynthRuntime] ℹ️ Hash \(latestHash) already applied; skipping hot update trigger")
        return
      }

      print("[ZynthRuntime] ♻️ Triggering hot update after \(trigger) message (hash: \(latestHash))")
      applyHotUpdate()
    }
    ...
    private func installDevServerGlobal(url: URL, token: String?) {
      let escaped = url.absoluteString
        ...
        .replacingOccurrences(of: "\"", with: "\\\"")
      runtime.evaluate(
        code: "globalThis.__ZYNTH_DEV_SERVER_URL = \"\(escaped)\";"
      )
      if let token {
        let escapedToken =
          token
          ...
          .replacingOccurrences(of: "\"", with: "\\\"")
        runtime.evaluate(
          code: "globalThis.__ZYNTH_DEV_SERVER_TOKEN = \"\(escapedToken)\";"
        )
      } else {
        runtime.evaluate(code: "delete globalThis.__ZYNTH_DEV_SERVER_TOKEN;")
      }
    }
  }
  ...
  extension ZynthRuntime {
    ...
    func loadDevBundleIfAvailable() -> Bool {
      ...
      print("[ZynthRuntime] 📦 loadDevBundleIfAvailable called")
      ensureMain { self.statusBar.showBundleLoading() }

      do {
        let bundle = try ZynthDevBundleFetcher.fetch(baseURL: devURL, token: devServerToken)
        lastDevBundle = bundle
        evaluateDevBundle(bundle.code, description: bundle.url.absoluteString)
        print("[ZynthRuntime] ✅ Bundle loaded successfully, showing status")
        DispatchQueue.main.async {
        ...
        if let cached = lastDevBundle {
          print("[ZynthRuntime] 💾 Falling back to cached dev bundle")
    ...
    private func evaluateDevBundle(_ code: String, description: String) {
      ...
      if lastDevBundle != nil {
        print("[ZynthRuntime] Resetting runtime and clearing UI for dev reload")
      ...
      print("[ZynthRuntime] Evaluated dev bundle from \(description)")
    ...
    public func refreshDevBundle() {
      guard devServerURL != nil else {
        NSLog("[ZynthRuntime] refreshDevBundle called without devServerURL")
        return
      }
      print("[ZynthRuntime] ⚡️ refreshDevBundle called from thread: \(Thread.current)")
    ...
    private func restartAfterReload() {
      ...
      print("[ZynthRuntime] Restarting app after dev reload with rootId", rootId)
      _ = runtime.callGlobal("__startApp", args: [rootId])
    }

    public func applyHotUpdate() {
      if isApplyingHotUpdate {
        print("[ZynthRuntime] ⏳ Hot update request ignored; another update is in progress")
        return
      }

      guard let devServerURL else {
        print("[ZynthRuntime] Hot update requested without dev server URL, doing full reload")
        DispatchQueue.main.async {
          self.refreshDevBundle()
        }
        return
      ...
      print("[ZynthRuntime] 🔥 Applying hot update...")
      ensureMain { self.statusBar.showUpdating() }

      let manifestCandidates = hotUpdateManifestPaths()
      print("[ZynthRuntime] 📦 Manifest candidates: \(manifestCandidates)")

      func attemptManifest(at index: Int) {
        guard index < manifestCandidates.count else {
          print("[ZynthRuntime] ❌ Manifest unavailable after trying all candidates")
          DispatchQueue.main.async {
        ...
        URLSession.shared.dataTask(with: request) { [weak self] data, response, error in
          ...
          if let error {
            print(
              "[ZynthRuntime] ❌ Manifest fetch failed from \(relativePath): \(error.localizedDescription)"
            )
          ...
          guard let httpResponse = response as? HTTPURLResponse else {
            print("[ZynthRuntime] ❌ Manifest response was not HTTP URL response")
            attemptManifest(at: index + 1)
            return
          }
          ...
          guard httpResponse.statusCode == 200 else {
            print(
              "[ZynthRuntime] ❌ Manifest status \(httpResponse.statusCode) from \(relativePath)"
            )
            attemptManifest(at: index + 1)
            return
          }
          ...
          guard let data else {
            print("[ZynthRuntime] ❌ Manifest returned empty body from \(relativePath)")
            attemptManifest(at: index + 1)
            return
          }
          ...
          guard let object = try? JSONSerialization.jsonObject(with: data) as? [String: Any]
            ...
            print("[ZynthRuntime] ❌ Manifest JSON parse failed from \(relativePath)")
            if let str = String(data: data, encoding: .utf8) {
              print("[ZynthRuntime] Raw response: \(str.prefix(200))")
            }
            attemptManifest(at: index + 1)
            return
          }

          print("[ZynthRuntime] ✅ Manifest loaded: \(object.keys)")
          ...
          if let cArray = object["c"] as? [String] {
            ...
            print("[ZynthRuntime] ❌ No chunks found in manifest payload: \(object)")
            attemptManifest(at: index + 1)
            return
          }
    ...
    private func fallbackToFullReload() {
      ...
      print("[ZynthRuntime] 🔄 Falling back to full bundle reload")
      self.refreshDevBundle()
    }
    ...
    private func fetchAndApplyHotUpdateChunks(
      ...
      guard !chunkIds.isEmpty else {
        print("[ZynthRuntime] ⚠️ No hot-update chunks to fetch, falling back to full reload")
        DispatchQueue.main.async {
          self.fallbackToFullReload()
        }
        return
      ...
      guard devServerURL != nil else {
        print("[ZynthRuntime] ❌ No dev server URL available")
        return
      }

      print("[ZynthRuntime] 📥 Fetching \(chunkIds.count) hot-update chunk(s): \(chunkIds)")
      ...
      func attemptFetch(
        ...
        print(
          "[ZynthRuntime]   Fetching (\(index + 1)/\(candidates.count)): \(chunkURL.absoluteString)")
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
          ...
          print(
            "[ZynthRuntime] ✅ Chunk \(chunkId) fetched from \(relativePath) (\(code.count) bytes)")
          lock.lock()
      ...
      group.notify(queue: .main) { [weak self] in
        ...
        print(
          "[ZynthRuntime] 📊 Chunk fetch complete: \(successCount) succeeded, \(errorCount) failed")

        if successCount == 0 {
          print("[ZynthRuntime] ❌ No chunks fetched successfully, performing full reload")
          self.fallbackToFullReload()
          return
        }

        if errorCount > 0 {
          print("[ZynthRuntime] ⚠️ Some chunks failed to fetch, but proceeding with available chunks")
        }

        // Execute chunks in order
        for chunkId in chunkIds {
          guard let code = chunkScripts[chunkId] else {
            print("[ZynthRuntime] ⏭️ Skipping chunk \(chunkId) (not fetched)")
            continue
          }

          print("[ZynthRuntime] ⚡️ Executing hot-update chunk: \(chunkId)")

          do {
            self.runtime.evaluate(code: code)
            print("[ZynthRuntime] ✅ Chunk \(chunkId) executed successfully")
          } catch {
            print("[ZynthRuntime] ❌ Failed to execute chunk \(chunkId): \(error)")
          }
        }
        ...
        print("[ZynthRuntime] 🎉 Hot update applied successfully!")
        self.statusBar.showBundleLoaded()
      }
    }
