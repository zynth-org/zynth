...
import com.zynth.kit.dev.ZynthDevBundle
import com.zynth.kit.dev.ZynthDevBundleFetcher
...
private const val TAG = "ZynthRuntime"

/**
 * Dev-only extension properties and methods for ZynthRuntime.
...
internal val ZynthRuntime.discoveryExecutor: ExecutorService
  get() = getOrCreateProperty("discoveryExecutor") {
    Executors.newSingleThreadExecutor { runnable ->
      Thread(runnable, "ZynthDevDiscovery").apply { isDaemon = true }
    }
  }

internal val ZynthRuntime.discoveryClient: OkHttpClient
...
internal val ZynthRuntime.bundleExecutor: ExecutorService
  get() = getOrCreateProperty("bundleExecutor") {
    Executors.newSingleThreadExecutor { runnable ->
      Thread(runnable, "ZynthBundleFetcher").apply { isDaemon = true }
    }
  }
...
internal var ZynthRuntime.isConnectingToDevServer: Boolean
...
internal var ZynthRuntime.lastDevBundle: ZynthDevBundle?
  get() = getProperty("lastDevBundle") as? ZynthDevBundle
...
internal val ZynthRuntime.statusBar: ZynthDevStatusBar
  get() = getOrCreateProperty("statusBar") { ZynthDevStatusBar(root) }
 ...
 * Called from ZynthRuntime.init in DEBUG builds only.
 */
// ============================================================================
// Public API Implementations
// ============================================================================
// These are the ONLY public dev methods. They are called from the main
// ZynthRuntime class wrappers.
...
private fun ZynthRuntime.savePersistedDevServerConfig(url: String, token: String?) {
  val context = root.context ?: return
  runCatching {
    val zynthDir = File(context.filesDir, ".zynth")
    if (!zynthDir.exists()) {
      zynthDir.mkdirs()
    }
    ...
    val devtoolsUrl =
      System.getProperty("ZYNTH_DEVTOOLS_URL")
        ?.takeIf { it.isNotBlank() }
        ?: existing?.optString("devtoolsUrl", "")?.takeIf { it.isNotBlank() }
    val devtoolsToken =
      System.getProperty("ZYNTH_DEVTOOLS_TOKEN")
        ?.takeIf { it.isNotBlank() }
...
private fun ZynthRuntime.loadPersistedDevServerConfig(): Pair<String?, String?>? {
...
private fun loadDevServerConfigFromAssets(context: android.content.Context): Pair<String?, String?>? {
  return runCatching {
    context.assets.open("zynth-dev-config.json").bufferedReader().use { reader ->
...
internal fun ZynthRuntime.handleDevMessageInternal(payload: String) {
  val json = try {
    ...
    adapter.callGlobal("__zynth_receiveHMRMessage", arrayOf(payload))
    return
  }
...
internal fun ZynthRuntime.configureDevServer() {
  ...
  System.getenv("ZYNTH_DEV_RUNTIME")
    ?.trim()
  ...
  if (!configured) {
    val envUrl = System.getenv("ZYNTH_DEV_SERVER_URL")?.takeUnless { it.isBlank() }
    val envToken = sanitizeToken(System.getenv("ZYNTH_DEV_SERVER_TOKEN"))
 ...
 */
fun ZynthRuntime.installHmrShim() {
  adapter.evaluate(
    """
    ...
    if (typeof globalThis.__zynth_refresh !== 'function') {
      globalThis.__zynth_refresh = function(payload) {
        // console.warn('[Zynth HMR] Refresh invoked with no runtime listener', payload && payload.type);
      };
 ...
 * Called from ZynthRuntime.destroy() in DEBUG builds only.
 */
fun ZynthRuntime.disconnectDevServer() {
...
internal fun ZynthRuntime.installDevServerGlobal(url: String, token: String?) {
  ...
  adapter.evaluate("globalThis.__ZYNTH_DEV_SERVER_URL = \"$escaped\";")
  val sanitizedToken = sanitizeToken(token)
  if (sanitizedToken != null) {
    val escapedToken = sanitizedToken.replace("\\", "\\\\").replace("\"", "\\\"")
    adapter.evaluate("globalThis.__ZYNTH_DEV_SERVER_TOKEN = \"$escapedToken\";")
  } else {
    adapter.evaluate("delete globalThis.__ZYNTH_DEV_SERVER_TOKEN;")
  }
}
...
private fun ZynthRuntime.autoDiscoverDevServer() {
...
private fun ZynthRuntime.probeDevServer(baseUrl: String): Boolean {
  return try {
    val request = Request.Builder()
      .url("$baseUrl/health")
      .get()
 ...
 */
fun ZynthRuntime.loadDevBundleIfAvailable(): Boolean {
...
private fun ZynthRuntime.fetchDevBundle(devUrl: String): ZynthDevBundle {
  return if (Looper.myLooper() == Looper.getMainLooper()) {
    val future: Future<ZynthDevBundle> = bundleExecutor.submit<ZynthDevBundle> {
      ZynthDevBundleFetcher.fetch(devUrl, devServerToken)
    }
    try {
      future.get()
...
private fun ZynthRuntime.evaluateDevBundle(bundle: ZynthDevBundle) {
...
private fun ZynthRuntime.scheduleDevBundleRetry() {
  if (pendingRetry) {
    return
  }
...
private fun ZynthRuntime.restartAfterReload() {
