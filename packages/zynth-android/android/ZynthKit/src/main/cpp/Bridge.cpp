...
constexpr const char *kLogTag = "ZynthHermesBridge";
...
constexpr const char *kZynthSharedValueKey = "__zynth_shared_value";
...
struct ZynthSharedValueAnimation {
  ZynthSharedAnimationKind kind = ZynthSharedAnimationKind::Timing;
...
static ZynthSharedEasing parseEasing(const std::string &name) {
...
static double applyEasing(ZynthSharedEasing easing, double t) {
...
/**
 * Unified diagnostics reporting function similar to iOS ZynthReportJSIError.
 * Reports errors through the ZynthDiagnostics system.
 */
void ZynthReportJSIError(
    facebook::jsi::Runtime & /* rt */, 
  ...
  // Call the JNI bridge to ZynthDiagnostics
  jclass zynthDiagClass = env->FindClass("com/zynth/kit/dev/ZynthDiagnosticsKt");
  if (zynthDiagClass) {
    jmethodID reportMethod = env->GetStaticMethodID(
        zynthDiagClass, 
        "zynthDiagnosticsReportJNI", 
        "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V");
    if (reportMethod) {
      env->CallStaticVoidMethod(zynthDiagClass, reportMethod, jPhase, jMessage, jStack);
    }
    env->DeleteLocalRef(zynthDiagClass);
  }
...
void installPlatformFlag(std::shared_ptr<RuntimeState> state) {
  ...
  rt.global().setProperty(rt, "__ZYNTH_PLATFORM", platform);
}
...
static void stepNativeAnimations(
  ...
  std::vector<ZynthStyleMapper> mappers;
...
void installAnimateBindings(std::shared_ptr<RuntimeState> state) {
  ...
  auto updateStyleMapper = Function::createFromHostFunction(
      ...
      [weakState](Runtime &runtime, const Value &, const Value *args, size_t count) -> Value {
        ...
        mapper.opacity = ZynthMappedValue();
        mapper.translateX = ZynthMappedValue();
        mapper.translateY = ZynthMappedValue();
        mapper.scale = ZynthMappedValue();
        mapper.scaleX = ZynthMappedValue();
        mapper.scaleY = ZynthMappedValue();
        mapper.rotate = ZynthMappedValue();
        mapper.rotateX = ZynthMappedValue();
        mapper.rotateY = ZynthMappedValue();
        mapper.skewX = ZynthMappedValue();
        mapper.skewY = ZynthMappedValue();
        mapper.perspective = ZynthMappedValue();
...
void installModules(std::shared_ptr<RuntimeState> state) {
  ...
  auto callFn = Function::createFromHostFunction(
    ...
    auto callSyncFn = Function::createFromHostFunction(
        ...
        [weakState](Runtime &runtime, const Value &, const Value *args, size_t count) -> Value {
            try {
                ...
                ZynthReportJSIError(runtime, error, "__modules.callSync");
                throw;
            } catch (const std::exception &ex) {
    ...
    rt.global().setProperty(rt, "__zynthCallSync", callSyncFn);
}
...
void installUnhandledPromiseReporting(std::shared_ptr<RuntimeState> state) {
  ...
  rt.evaluateJavaScript(buffer, "zynth-unhandled.js");
}
...
void evaluateString(
  ...
  try {
    ...
    ZynthReportJSIError(*runtime, err, "Evaluate");
    throw;
  } catch (const std::exception &ex) {
    JniEnv env;
    if (env.valid()) {
...
void emitEvent(
  ...
  try {
    ...
    if (!global.hasProperty(rt, "ZynthNativeEmitter")) {
      BRIDGE_LOG(ANDROID_LOG_WARN, "ZynthNativeEmitter missing when emitting %s", eventName.c_str());
      return;
    }

    auto emitterValue = global.getProperty(rt, "ZynthNativeEmitter");
    if (!emitterValue.isObject()) {
      BRIDGE_LOG(ANDROID_LOG_WARN, "ZynthNativeEmitter is not an object for event %s", eventName.c_str());
      return;
    }

    auto emitterObj = emitterValue.asObject(rt);
    if (!emitterObj.hasProperty(rt, "emit")) {
      BRIDGE_LOG(ANDROID_LOG_WARN, "ZynthNativeEmitter.emit missing for event %s", eventName.c_str());
      return;
    }

    auto emitValue = emitterObj.getProperty(rt, "emit");
    if (!emitValue.isObject()) {
      BRIDGE_LOG(ANDROID_LOG_WARN, "ZynthNativeEmitter.emit is not callable for event %s", eventName.c_str());
      return;
    }
...
Java_com_zynth_kit_runtime_ZynthBridge_nativeInstallBindings(JNIEnv *, jclass, jobject, jobject) {
  __android_log_print(ANDROID_LOG_INFO, "ZynthHermesBridge", "Fallback ZynthBridge installBindings invoked (no-op)");
}
