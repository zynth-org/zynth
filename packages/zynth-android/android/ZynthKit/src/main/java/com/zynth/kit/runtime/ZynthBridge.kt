...
object ZynthBridge {
  private const val TAG = "ZynthBridge"
  ...
  fun install(adapter: JSRuntimeAdapter, manager: ZynthUIManager) {
    try {
  ...
  private fun installFallback(adapter: JSRuntimeAdapter, manager: ZynthUIManager) {
  ...
  @JvmStatic
  private external fun nativeInstallBindings(adapter: JSRuntimeAdapter, manager: ZynthUIManager)
}
