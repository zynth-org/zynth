...
internal class FrameCommitCoordinator(
  ...
  private val mode: BarrierMode = run {
    when (System.getProperty("zynth.frameBarrier.mode")?.lowercase()) {
  ...
  fun onMutationsQueued(reason: String? = null) {
    ...
    if (!isBlocking) {
      isBlocking = true
      logDebug("ZynthFrame", "surface=$surfaceId commit barrier start${reason?.let { " ($it)" } ?: ""} mode=$mode")
  ...
  fun onFlushComplete(hasPendingWork: Boolean) {
    ...
    logDebug("ZynthFrame", "surface=$surfaceId releasing block after $pendingTransactions tx")
  ...
  private fun scheduleTimeout() {
    val runnable = Runnable {
      if (isBlocking) {
        Log.w("ZynthFrame", "surface=$surfaceId commit barrier timed out, forcing release")
