...
import com.zynth.kit.runtime.modules.DimensionsModule
import com.zynth.kit.runtime.modules.FetchModule
import com.zynth.kit.runtime.modules.BackHandlerModule
...
class ZynthRuntime(
  internal val root: ZynthRootView,  // internal for dev extension access
  ...
  private val moduleExecutor: ExecutorService = Executors.newSingleThreadExecutor { runnable ->
    Thread(runnable, "ZynthModuleInvoker-$id").apply { isDaemon = true }
  }
  private val installedModules = LinkedHashMap<String, ZynthModule>()
  internal var lastRootId: Int? = null  // internal for dev extension access
  
  // Core UI manager
  internal val manager = ZynthUIManager(  // internal for dev extension access
    root,
    eventDispatcher = { id, name -> dispatchHandler(id, name) },
    handlerListener = { id, name, handlerRef -> onHandlerAttached(id, name, handlerRef) },
  )
  fun getUIManager(): ZynthUIManager = manager
  fun getRootView(): ZynthRootView = root
  ...
  fun installDefaultModules() {
    val modulesToInstall = mutableListOf<ZynthModule>()
    ...
    // Note: FontModule is installed by @zynth/apis package via ZynthAPIs.initialize()
  ...
  fun emitEvent(name: String, payload: Any?) {
    when (val runtimeAdapter = adapter) {
      ...
      else -> runtimeAdapter.callGlobalAsync("ZynthNativeEmitter.emit", arrayOf(name, payload))
    }
  }
  ...
  fun registerSurface(rootView: ZynthRootView) {
  ...
  private fun installConsolePolyfills(runtimeAdapter: JSRuntimeAdapter) {
    ...
    runtimeAdapter.setGlobalFunction("__zynthConsoleTimeStart") { args ->
      val label = args.firstOrNull()?.toString() ?: "default"
      timers[label] = SystemClock.elapsedRealtimeNanos()
      null
    }

    runtimeAdapter.setGlobalFunction("__zynthConsoleTimeLog") { args ->
      val label = args.firstOrNull()?.toString() ?: "default"
    ...
    runtimeAdapter.setGlobalFunction("__zynthConsoleTimeEnd") { args ->
      val label = args.firstOrNull()?.toString() ?: "default"
    ...
    runtimeAdapter.setGlobalFunction("__zynthPerformanceNow") { _ ->
    ...
    runtimeAdapter.evaluate(
      ...
      (function() {
        globalThis.console = globalThis.console || {};
        const hasNativeTimeStart = typeof __zynthConsoleTimeStart === 'function';
        const hasNativeTimeLog = typeof __zynthConsoleTimeLog === 'function';
        const hasNativeTimeEnd = typeof __zynthConsoleTimeEnd === 'function';
        const nativeNow = typeof __zynthPerformanceNow === 'function'
          ? __zynthPerformanceNow
  ...
  private fun installFallbackBridge() {
    ZynthBridge.install(adapter, manager)
  ...
  private fun dispatchHandler(id: Int, name: String) {
    logDebug("ZynthUI", "dispatchHandler called for id=$id name=$name")
    val key = id to name
    val handler = handlerMap[key]
    val runtimeAdapter = adapter
    logDebug("ZynthUI", "Handler function found: ${handler != null}")
    when {
      handler is HandlerRef.Hermes && runtimeAdapter is HermesAdapter -> {
        logDebug("ZynthUI", "Dispatching Hermes handler $handler for node $id")
        runtimeAdapter.invokeHandler(handler.handlerId, id, name)
      }
      else -> logDebug("ZynthUI", "No valid handler found for $key")
    }
  }
  ...
  companion object {
     ...
     * Call this before creating any ZynthRuntime instances.
     */
    @JvmStatic
