...
let nextId = 1;
const LAYOUT_OBSERVER_KEY = "__zynth_layout_observer";
const LAYOUT_RESIZE_KEY = "__zynth_layout_resize";
const LAYOUT_CALLBACK_KEY = "__zynth_layout_callback";
...
const GLOBAL_REGISTRY_KEY = "__ZYNTH_WEB_REGISTRY__";
...
export function registerComponent(
  ...
  registerWebComponent(type, {
    create: (initialProps) => {
      const container = document.createElement("zynth-web-host");
    ...
    insertChild: (parent, child, anchor) => {
      let slot = parent.querySelector("[data-zynth-slot]") as HTMLElement | null;
      
      // Support named slots: if child has a 'slot' attribute, find matching data-zynth-slot
      if (child instanceof HTMLElement && child.getAttribute("slot")) {
        const name = child.getAttribute("slot");
        const namedSlot = parent.querySelector(`[data-zynth-slot="${name}"]`) as HTMLElement | null;
...
export function createWebHost(): Host {
  return {
    createRootContainer(container: unknown): HostNode {
      ...
      rootElement.classList.add("zynth-root");
    ...
    createNode(type, props): HostNode {
      ...
      (element as any).__zynth_props = props || {};
        ...
        for (const [rawKey, value] of Object.entries(props)) {
          ...
          if (key === "onLayout" && typeof value === "function") {
            ...
            if (eventName === "press") {
              ...
              // These are handled by the generic listeners above using __zynth_props
            } else {
