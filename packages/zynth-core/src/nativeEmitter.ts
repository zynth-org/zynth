...
export type ZynthNativeEmitterBridge = {
...
function emitToListeners(eventName: string, payload: unknown): void {
  ...
  for (const listener of snapshot) {
    try {
      ...
      console.error(
        `[ZynthNativeEmitter] listener for ${eventName} threw`,
        error
      );
    }
  }
...
const emitterBridge: ZynthNativeEmitterBridge = {
...
export function ensureNativeEmitter(): ZynthNativeEmitterBridge {
  const g = getGlobalObject();
  const existing = g.ZynthNativeEmitter as ZynthNativeEmitterBridge | undefined;
  ...
  if (
    ...
    Object.defineProperty(g, "ZynthNativeEmitter", {
      configurable: true,
      enumerable: false,
      writable: true,
      value: emitterBridge,
...
export class NativeEventEmitter {
  private readonly target: ZynthNativeEmitterBridge;

  constructor(target: ZynthNativeEmitterBridge = ensureNativeEmitter()) {
    this.target = target;
  }
