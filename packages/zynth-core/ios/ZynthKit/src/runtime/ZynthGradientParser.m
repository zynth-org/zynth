...
static NSArray<NSString *> *ZynthSplitGradientArgs(NSString *raw) {
...
static CGFloat ZynthNormalizeAngle(CGFloat angle) {
  CGFloat a = fmod(angle, 360.0);
  if (a < 0) a += 360.0;
  return a;
}

static CGFloat ZynthParseDirection(NSString *dir, BOOL *ok) {
...
static CGFloat ZynthParseAngleToken(NSString *token, BOOL *ok) {
  ...
  if ([trimmed hasPrefix:@"to "]) {
    BOOL dirOk = NO;
    CGFloat angle = ZynthParseDirection([trimmed substringFromIndex:3], &dirOk);
    *ok = dirOk;
    return angle;
  }
...
static NSNumber *_Nullable ZynthParsePosition(NSString *raw) {
...
static ZynthGradientStop *_Nullable ZynthParseStop(NSString *token) {
  NSString *trimmed = [token stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
  ...
  if (!color) return nil;
  NSNumber *pos = positionPart ? ZynthParsePosition(positionPart) : nil;
...
+ (nullable ZynthLinearGradient *)parse:(id)value {
  ...
  NSArray<NSString *> *tokens = ZynthSplitGradientArgs(inside);
  if (tokens.count < 2) return nil;

  CGFloat angle = 180.f;
  NSUInteger startIndex = 0;
  BOOL angleOk = NO;
  angle = ZynthParseAngleToken(tokens.firstObject, &angleOk);
  if (angleOk) {
    startIndex = 1;
  } else {
    angle = 180.f;
  ...
  for (NSUInteger i = startIndex; i < tokens.count; i++) {
    ZynthGradientStop *stop = ZynthParseStop(tokens[i]);
    if (stop) [stops addObject:stop];
  }
  if (stops.count < 2) return nil;

  return [[ZynthLinearGradient alloc] initWithAngle:ZynthNormalizeAngle(angle) stops:stops];
}
