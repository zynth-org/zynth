...
static inline UIColor *ZynthDefaultShadowColor(void) {
  return [[UIColor blackColor] colorWithAlphaComponent:0.25];
}

@implementation ZynthShadowLayer
...
- (instancetype)initWithOffsetX:(CGFloat)offsetX
  ...
  if (self = [super init]) {
    ...
    _spread = spread;
    _color = color ?: ZynthDefaultShadowColor();
    _inset = inset;
  }
  return self;
}
...
@implementation ZynthShadowParser
...
+ (nullable NSArray<ZynthShadowLayer *> *)parse:(id)value {
  ...
  if ([value isKindOfClass:[NSString class]]) {
    ...
    return [self parseStringList:trimmed];
  }
  if ([value isKindOfClass:[NSArray class]]) {
    NSMutableArray<ZynthShadowLayer *> *layers = [NSMutableArray array];
    for (id item in (NSArray *)value) {
      ZynthShadowLayer *layer = nil;
  ...
  if ([value isKindOfClass:[NSDictionary class]]) {
    ZynthShadowLayer *layer = [self parseObject:(NSDictionary *)value];
...
+ (nullable NSArray<ZynthShadowLayer *> *)fromReactNativeColor:(nullable UIColor *)color
  ...
  UIColor *base = color ?: ZynthDefaultShadowColor();
  CGFloat r = 0, g = 0, b = 0, a = 1;
  if (![base getRed:&r green:&g blue:&b alpha:&a]) {
    base = ZynthDefaultShadowColor();
    [base getRed:&r green:&g blue:&b alpha:&a];
  }
  UIColor *resolved = [UIColor colorWithRed:r green:g blue:b alpha:a * alpha];

  ZynthShadowLayer *layer = [[ZynthShadowLayer alloc] initWithOffsetX:(offsetX ? offsetX.doubleValue : 0.f)
...
+ (nullable ZynthShadowLayer *)parseObject:(NSDictionary *)dict {
  ...
  UIColor *color = colorString ? [ZynthColorParser parseColor:colorString] : nil;

  return [[ZynthShadowLayer alloc] initWithOffsetX:(offsetX ? offsetX.doubleValue : 0.f)
                                         offsetY:(offsetY ? offsetY.doubleValue : 0.f)
                                      blurRadius:(blur ? blur.doubleValue : 0.f)
                                          spread:(spread ? spread.doubleValue : 0.f)
                                           color:color ?: ZynthDefaultShadowColor()
                                           inset:(insetVal ? insetVal.boolValue : NO)];
}

+ (NSArray<ZynthShadowLayer *> *)parseStringList:(NSString *)value {
  NSMutableArray<ZynthShadowLayer *> *layers = [NSMutableArray array];
  for (NSString *part in [self splitByTopLevelCommas:value]) {
    ZynthShadowLayer *layer = [self parseShadowString:part];
    if (layer) [layers addObject:layer];
  }
  return layers.count > 0 ? layers : nil;
}

+ (nullable ZynthShadowLayer *)parseShadowString:(NSString *)raw {
  ...
  if (colorMatch) {
    ...
    color = [ZynthColorParser parseColor:token];
  ...
  for (NSString *token in tokens) {
    ...
    if (!color) {
      UIColor *parsed = [ZynthColorParser parseColor:token];
  ...
  UIColor *resolved = color ?: ZynthDefaultShadowColor();
  return [[ZynthShadowLayer alloc] initWithOffsetX:offsetX
                                         offsetY:offsetY
                                      blurRadius:blur
                                          spread:spread
                                           color:resolved
