...
zynth_root = File.expand_path("../../..", __dir__)
  ...
  if HermesEngineSourceType::isPrebuilt(source_type) then
    ...
    spec.subspec 'Pre-built' do |ss|
      ...
      if ENV["RCT_HERMES_V1_ENABLED"] == "0"
        ...
        ss.source_files = ["destroot/include/hermes/**/*.h", "destroot/include/jsi/*.h"]
        ss.public_header_files = ["destroot/include/hermes/**/*.h", "destroot/include/jsi/*.h"]
      end
    ...
    debug_tarball = "#{artifacts_dir_path}/hermes-ios-#{version}-debug.tar.gz"
    release_tarball = "#{artifacts_dir_path}/hermes-ios-#{version}-release.tar.gz"
    spec.prepare_command = <<~EOS
      set -e
      if [ ! -d "destroot" ]; then
        if [ -f "#{debug_tarball}" ]; then
          tar -xzf "#{debug_tarball}" -C .
        elif [ -f "#{release_tarball}" ]; then
          tar -xzf "#{release_tarball}" -C .
        else
          echo "[Hermes] Missing prebuilt tarball in #{artifacts_dir_path}"
          exit 1
        fi
      fi
    EOS
    ...
    if source_type == HermesEngineSourceType::DOWNLOAD_PREBUILD_RELEASE_TARBALL &&
        File.exist?(File.join(react_native_path, "scripts", "xcode", "with-environment.sh")) &&
        File.exist?(File.join(react_native_path, "sdks", "hermes-engine", "utils", "replace_hermes_version.js"))
      spec.script_phase = {
    ...
    if File.exist?(File.join(react_native_path, "sdks", "hermes-engine", "utils", "create-dummy-hermes-xcframework.sh"))
