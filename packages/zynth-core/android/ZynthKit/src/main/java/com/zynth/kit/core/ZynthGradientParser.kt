...
data class ZynthGradientStop(val color: Int, val position: Float?)
data class ZynthLinearGradient(val angle: Float, val stops: List<ZynthGradientStop>)

object ZynthGradientParser {

  fun parse(value: Any?): ZynthLinearGradient? {
  ...
  private fun parseLinearGradient(raw: String): ZynthLinearGradient? {
    ...
    val stops = mutableListOf<ZynthGradientStop>()
    for (i in startIndex until tokens.size) {
      parseStop(tokens[i])?.let { stops.add(it) }
    }
    if (stops.size < 2) return null

    return ZynthLinearGradient(normalizeAngle(angle), stops)
  }
  ...
  private fun parseStop(token: String): ZynthGradientStop? {
    val trimmed = token.trim()
    ...
    val color = ZynthColorParser.parse(colorPart) ?: return null
    val position = positionPart?.let { parsePosition(it) }
    return ZynthGradientStop(color, position)
  }
