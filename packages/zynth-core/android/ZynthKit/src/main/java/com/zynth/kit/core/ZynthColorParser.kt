...
object ZynthColorParser {
    ...
    private fun parseHwb(value: String): Int? {
        ...
        return ColorUtils.setAlphaComponent(finalColor, alpha)
    }

    private fun parseAlpha(raw: String?): Int {
        if (raw.isNullOrBlank()) return 255
        val trimmed = raw.trim()
        if (trimmed.endsWith("%")) {
            val percent = trimmed.removeSuffix("%").toFloatOrNull() ?: return 255
            return (percent / 100f * 255f).roundToInt().coerceIn(0, 255)
        }
        val num = trimmed.toFloatOrNull() ?: return 255
        return if (num <= 1f) {
            (num * 255f).roundToInt().coerceIn(0, 255)
        } else {
            num.roundToInt().coerceIn(0, 255)
        }
    }
}
