...
`@zynth/core` now provides an explicit style graph API for native-first styling.

```ts
import { StyleGraph } from "@zynth/core";

const styles = StyleGraph.create({
  card: {
    width: "100%",
    borderRadius: 24,
    borderWidth: 1,
...
const cardStyle = StyleGraph.bind(styles.card, {
  borderColor: () => palette.border(),
  backgroundColor: () => (state.active() ? "#4f46e5" : "#1f2937"),
});
```

Use with primitives as regular `style` values:
...
- Static style refs are stable and reusable.
- Dynamic bindings patch only changed keys.
- Removed keys are explicitly reset (`null`) on native targets.
- Existing plain style objects and arrays remain fully supported.
