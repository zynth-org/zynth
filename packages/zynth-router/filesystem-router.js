...
const DEFAULT_MODULE_ID = "@zynthjs/router/fs-routes";
...
export function routerFileSystemGeneratedModule(options = {}) {
  const normalized = {
...
function buildRouteManifestModule(root, outputPath) {
  ...
  const header = [
    "/* eslint-disable */",
    "// Auto-generated by @zynthjs/router. Do not edit manually.",
  ];
...
function assertUniqueChildRouteNames(directory) {
  ...
  for (const screen of directory.screenFiles) {
    if (seen.has(screen.routeName)) {
      throw new Error(
        `[router/fs] Duplicate route name '${screen.routeName}' in '${directory.directoryPath}'.`,
      );
    }
  ...
  for (const child of directory.children) {
    ...
    if (seen.has(childName)) {
      throw new Error(
        `[router/fs] Duplicate route name '${childName}' in '${directory.directoryPath}'.`,
      );
    }
    seen.add(childName);
