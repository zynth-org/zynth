...
function findWorkspaceRoot(startDir) {
  ...
  while (true) {
    ...
    if (parent === current) {
      throw new Error('Workspace root not found. Run this command inside a Rune workspace.');
    }
    current = parent;
  }
}
...
function findAppDirectory(startDir) {
  ...
  while (true) {
    ...
    const parent = path.dirname(current);
    if (parent === current) {
      throw new Error('Could not determine app directory. Pass --app <relative-path>.');
    }
    current = parent;
  }
}
...
function startIOSLogs(config) {
  try {
    const child = spawn('xcrun', [
      ...
      'debug',
      '--predicate',
      `process == "${config.appNameCapitalized}" AND (eventMessage BEGINSWITH "[Rune]" OR eventMessage CONTAINS "JS Error")`,
    ], {
      stdio: 'inherit',
    });
    const dispose = () => {
...
function devIOS(root, appDir) {
  ...
  const appBundle = path.join('.build', 'Build', 'Products', 'Debug-iphonesimulator', `${config.appNameCapitalized}.app`);
  runCommand('xcrun', ['simctl', 'bootstatus', 'booted', '-b']);
  runCommand('xcrun', ['simctl', 'install', 'booted', appBundle], { cwd: iosDir });
...
function devAndroid(root, appDir) {
  ...
  if (!devices.length) {
    console.warn('⚠️  No Android devices or emulators detected. Skipping install.');
    console.warn('    Install/launch manually with an emulator or device when available.');
    return;
  }

  runCommand('./gradlew', [':app:installDebug'], { cwd: androidDir });
  runCommand('adb', ['shell', 'am', 'start', '-n', `${config.bundleId}/.MainActivity`]);
}

function resetIOS(appDir) {
  const targets = [
    path.join(appDir, 'ios'),
  ...
  console.log('🧹 Removed iOS build artifacts. Run `rune prebuild ios` to regenerate.');
}
...
function resetAndroid(appDir) {
  ...
  console.log('🧹 Removed Android build artifacts. Run `rune prebuild android` to regenerate.');
}
...
function bundle(scope, root) {
  const runWorkspaceBuild = (workspace) => {
    ...
    runCommand('yarn', ['workspace', workspace.name, 'run', 'build'], { cwd: root });
  };
...
function run(rawArgs) {
  ...
  if (['prebuild', 'dev', 'reset'].includes(command)) {
    ...
    if (!platform || !['ios', 'android'].includes(platform)) {
      console.error(`Invalid or missing platform for ${command}. Expected ios or android.`);
      process.exit(1);
    }
    const appDir = args.flags.app ? path.resolve(cwd, args.flags.app) : findAppDirectory(cwd);
