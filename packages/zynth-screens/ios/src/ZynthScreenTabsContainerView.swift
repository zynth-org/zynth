...
struct ZynthNativeTabBarOptions: Equatable {
  var visible: Bool
  ...
  static let `default` = ZynthNativeTabBarOptions(
    visible: true,
    backgroundColor: nil,
...
struct ZynthNativeTabBarItem: Equatable {
  var key: String
  ...
  var hidden: Bool
  var icon: ZynthNativeTabIcon?
}

enum ZynthNativeTabIcon: Equatable {
  case descriptor(ZynthNativeTabBarIconDescriptor)
  case surface(routeKey: String)
}

struct ZynthNativeTabBarIconDescriptor: Equatable {
...
@objcMembers
public final class ZynthScreenTabsContainerView: UIView, UITabBarControllerDelegate {
  ...
  private var controllerMap: [ObjectIdentifier: ZynthTabContentViewController] = [:]
  private var tabDescriptors: [ZynthNativeTabBarItem] = [] {
    didSet { updateTabBarItems() }
  }
  private var tabBarOptions: ZynthNativeTabBarOptions = .default {
    didSet {
  ...
  private var tabAnimationType: ZynthScreenAnimation = .none
  ...
  public func setTabAnimationType(_ value: NSString?) {
    ...
    let newType: ZynthScreenAnimation
    if let stringValue, !stringValue.isEmpty {
      newType = ZynthScreenAnimation(string: stringValue)
    } else {
      newType = .none
    }
  ...
  public func setTabBarOptions(from dictionary: NSDictionary?) {
    var options = ZynthNativeTabBarOptions.default
    if let dict = dictionary as? [String: Any] {
      ...
      if let background = dict["backgroundColor"] as? String {
        options.backgroundColor = UIColor.zynth_color(from: background)
      }
      if let activeTint = dict["activeTintColor"] as? String {
        options.activeTintColor = UIColor.zynth_color(from: activeTint)
      }
      if let inactiveTint = dict["inactiveTintColor"] as? String {
        options.inactiveTintColor = UIColor.zynth_color(from: inactiveTint)
      }
  ...
  public func setTabItems(from array: NSArray?) {
    ...
    let descriptors: [ZynthNativeTabBarItem] = objects.compactMap { element in
    ...
    for (position, button) in buttonsToIterate.enumerated() {
      ...
      case .surface(let routeKey):
        ...
        if let existing = iconHostEntries[key] {
          ...
          let normalHost = ZynthTabIconHostView()
          let selectedHost = ZynthTabIconHostView()
  ...
  private func attachIconHost(_ host: ZynthTabIconHostView, to container: UIView, targetView: UIView?) {
  ...
  private func findFirstLabel(in view: UIView) -> UILabel? {
    ...
    if view is ZynthTabIconHostView {
      return nil
    }
    if let label = view as? UILabel, let text = label.text, !text.isEmpty {
      return label
  ...
  private func updateTabBarItems() {
    ...
    for (index, controller) in controllers.enumerated() {
      guard let tabController = controller as? ZynthTabContentViewController else { continue }
  ...
  private func configureTabBarItem(
    for controller: ZynthTabContentViewController,
    descriptor: ZynthNativeTabBarItem?,
    index: Int
  ) {
  ...
  private func makeImage(for descriptor: ZynthNativeTabBarIconDescriptor) -> UIImage? {
...
private final class NativeTabIconHostEntry {
  ...
  init(normalHost: ZynthTabIconHostView, selectedHost: ZynthTabIconHostView, index: Int, routeKey: String) {
...
private extension ZynthNativeTabBarItem {
  init?(dictionary: [String: Any]) {
    ...
    if let badgeColor = dictionary["badgeColor"] as? String {
      self.badgeColor = UIColor.zynth_color(from: badgeColor)
    }
    ...
    if let iconDict = dictionary["icon"] as? [String: Any],
      ...
      if type == "surface", let routeKey = iconDict["routeKey"] as? String {
        ...
        var descriptor = ZynthNativeTabBarIconDescriptor()
