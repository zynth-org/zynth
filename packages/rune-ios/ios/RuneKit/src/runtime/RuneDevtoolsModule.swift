...
final class RuneDevtoolsModule: RuneModule {
  let name: String = "Devtools"

#if DEBUG
  private let client = RuneDevtoolsClient()
#endif

  func initialize() {
#if DEBUG
    let env = ProcessInfo.processInfo.environment
    if let urlString = env["RUNE_DEVTOOLS_URL"], let url = URL(string: urlString) {
      let token = env["RUNE_DEVTOOLS_TOKEN"]
...
#if DEBUG
final class RuneDevtoolsClient: NSObject {
  private let queue = DispatchQueue(label: "dev.rune.devtools")
