...
static const CFTimeInterval kRunePressableDefaultDoublePressWindowMs = 250.0;

@interface RunePressableView ()
...
- (instancetype)initWithFrame:(CGRect)frame {
  if (self = [super initWithFrame:frame]) {
    ...
    _pressEffect = @"none";
    _doublePressWindowMs = kRunePressableDefaultDoublePressWindowMs;
    _lastCommandSeq = -1;
    _delayPressInMs = 0;
    _delayPressOutMs = 0;
    _delayLongPressMs = kRunePressableDefaultLongPressMs;
...
- (void)rune_setPressRetentionOffset:(NSNumber *)offset {
  self.pressRetentionOffset = offset ? offset.doubleValue : kRunePressableDefaultRetention;
}
...
- (void)rune_setDelayPressIn:(NSNumber *)delay {
  self.delayPressInMs = delay ? delay.doubleValue : 0;
}

- (void)rune_setDelayPressOut:(NSNumber *)delay {
  self.delayPressOutMs = delay ? delay.doubleValue : 0;
}

- (void)rune_setDelayLongPress:(NSNumber *)delay {
  self.delayLongPressMs = delay ? delay.doubleValue : kRunePressableDefaultLongPressMs;
}
...
- (void)rune_setDoublePressWindow:(NSNumber *)window {
  self.doublePressWindowMs = window ? MAX(0.0, window.doubleValue) : kRunePressableDefaultDoublePressWindowMs;
}

- (void)rune_setActivateKeys:(NSArray<NSString *> *)keys {
