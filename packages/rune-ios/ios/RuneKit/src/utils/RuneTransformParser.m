#import "RuneTransformParser.h"

@interface RuneTransformParser ()
...
@implementation RuneTransformParser
...
+ (CGFloat)parseAngle:(id)value {
    if ([value isKindOfClass:[NSNumber class]]) {
        return [value doubleValue]; // Assume radians if number
    }
    NSString *s = (NSString *)value;
    if ([s hasSuffix:@"deg"]) {
        NSString *num = [s substringToIndex:s.length - 3];
        return [num doubleValue] * M_PI / 180.0;
    }
    if ([s hasSuffix:@"rad"]) {
         NSString *num = [s substringToIndex:s.length - 3];
