...
A lightweight, memory-based navigation router for Rune apps. Inspired by React Navigation, built for SolidJS, and powered by `@rune/screens` for smooth transitions.

## Overview

`@rune/memory-router` provides a familiar navigation API similar to React Navigation, but designed specifically for Rune's architecture. It uses pure View-based screen primitives (via `@rune/screens`) instead of native navigation controllers, making it ideal for:
...
## Comparison with @rune/router

| Feature               | @rune/memory-router          | @rune/router                       |
| --------------------- | ---------------------------- | ---------------------------------- |
| Navigation approach   | Memory-based, JS-driven      | Native navigation controllers      |
| Screen transitions    | `@rune/screens` (View-based) | Native Fragments/UIViewControllers |
...
See the main [Rune repository](https://github.com/x64Bits/rune) for contribution guidelines.
