...
const androidInitProviderPath = join(
  androidJavaDir,
  "ZynthIconsInitProvider.java"
);
...
async function emitPackSource(
  ...
  const dtsHeader =
    ...
    'import type { TextProps } from "@zynthjs/components";\n' +
    'type IconProps = TextProps & { size?: number | string; color?: string; title?: string };\n\n';

  const dtsBody = icons
    .map(
      (icon) =>
        `export declare const ${icon.name}: (props: IconProps) => Element;`
    )
    .join("\n");
...
async function cleanAssetsFonts() {
  ...
  await mkdir(iosFontsDir, { recursive: true });
  await mkdir(androidFontsDir, { recursive: true });
  const cleanFontDir = async (dir: string) => {
    const entries = await readdir(dir, { withFileTypes: true });
  ...
  await Promise.all([
    cleanFontDir(assetsFontsDir),
    cleanFontDir(iosFontsDir),
    cleanFontDir(androidFontsDir),
  ]);
}
...
async function emitAndroidInitProvider(packs: string[]) {
  const header =
    "// AUTO-GENERATED by packages/zynth-icons/scripts/generate.ts. Do not edit.\n";
  const fontFamilies = packs
    .map((pack) => `    "ZynthIcons${pack.toUpperCase()}"`)
    .join(",\n");

  const contents = `${header}package dev.zynth.icons;
...
`;
  ...
  await mkdir(androidJavaDir, { recursive: true });
  await writeFile(androidInitProviderPath, contents, "utf8");
}
...
async function main() {
  ...
  await emitWebEntry();
  await emitAndroidInitProvider(packs);
