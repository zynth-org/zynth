...
This template provides a complete starting point for creating a new native module package in the Zynth framework.
...
```bash
cp -r packages/zynth-templates/native-module packages/zynth-my-module
cd packages/zynth-my-module
```
...
```tsx
import { MyModuleProvider, useMyModuleState } from "@zynth/my-module";
...
```
native-module/
├── package.json              # Package metadata with zynthNative config
