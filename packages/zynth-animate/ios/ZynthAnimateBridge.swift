...
private enum ZynthAnimateEasing: String {
  case linear
  case ease
  case easeIn
  case easeOut
...
private struct ZynthAnimatedStyle {
  var opacity: CGFloat?
...
private struct ZynthResolvedStyle {
  var opacity: CGFloat
  var translateX: CGFloat
  var translateY: CGFloat
  var scaleX: CGFloat
...
private struct ZynthResolvedKeyframe {
  let at: CGFloat
  let style: ZynthResolvedStyle
  let easing: ZynthAnimateEasing?
}

private struct ZynthStyleAnimation {
  let nodeId: Int
  let animationId: Int
  let phase: String
  let from: ZynthResolvedStyle
  let to: ZynthResolvedStyle
  let frames: [ZynthResolvedKeyframe]?
...
final class ZynthAnimateBridge: NSObject, ZynthModule {
  ...
  private func handleStartTransition(_ args: ZynthArgs) throws -> [String: Any] {
    ...
    runOnMain { [weak self] in
      ...
      guard let node = runtime.uiManager.zynth_node(forId: NSNumber(value: nodeId)) else { return }
      let view = node.view

      let resolvedFrames: [ZynthResolvedKeyframe]? = {
        guard !frameSpecs.isEmpty else { return nil }
        return frameSpecs
          .map { frame in
            ZynthResolvedKeyframe(
      ...
      let animation = ZynthStyleAnimation(
        nodeId: nodeId,
  ...
  @objc private func step(link: CADisplayLink) {
    ...
    for animation in items {
      ...
      guard let runtime = runtime,
            let node = runtime.uiManager.zynth_node(forId: NSNumber(value: animation.nodeId)) else {
  ...
  private func finishAnimation(_ animation: ZynthStyleAnimation) {
    animations.removeValue(forKey: animation.nodeId)
    if let runtime = runtime,
      let node = runtime.uiManager.zynth_node(forId: NSNumber(value: animation.nodeId)) {
      applyStyle(animation.to, to: node.view)
    }
    if let runtime = runtime {
      runtime.emitEvent(
        name: "ZynthAnimate:transitionEnd",
        payload: [
  ...
  private func applyStyle(_ style: ZynthResolvedStyle, to view: UIView) {
  ...
  private func interpolate(
    from: ZynthResolvedStyle,
    to: ZynthResolvedStyle,
    progress: Double
  ) -> ZynthResolvedStyle {
    let t = CGFloat(max(0.0, min(1.0, progress)))
    return ZynthResolvedStyle(
  ...
  private func resolveStyle(
    from: ZynthAnimatedStyle?,
    to: ZynthAnimatedStyle?,
    view: UIView
  ) -> ZynthResolvedStyle {
  ...
  private func parseStyle(_ value: Any?) -> ZynthAnimatedStyle? {
    guard let dict = value as? [String: Any] ?? (value as? NSDictionary as? [String: Any]) else {
      return nil
    }

    var style = ZynthAnimatedStyle()
  ...
  private struct ZynthKeyframeSpec {
    let at: Double
    let style: ZynthAnimatedStyle?
    let easing: ZynthAnimateEasing?
  }

  private func parseKeyframes(_ value: Any?) -> [ZynthKeyframeSpec] {
    let list: [Any]
    ...
    for entry in list {
      ...
      let easing = easingName.flatMap { ZynthAnimateEasing(rawValue: $0) }
      frames.append(ZynthKeyframeSpec(at: at, style: style, easing: easing))
    }
    return frames
  }
