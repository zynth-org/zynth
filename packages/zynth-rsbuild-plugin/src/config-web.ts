...
const DEFAULT_WEB_CONFIG: RsbuildConfig = {
  source: {
    ...
    define: {
      "process.env.ZYNTH_PLATFORM": JSON.stringify("web"),
      __ZYNTH_PLATFORM__: JSON.stringify("web"),
      "globalThis.__ZYNTH_PLATFORM__": JSON.stringify("web"),
    },
  },
  output: {
    distPath: {
...
function createWebBabelPlugin(babelOptions: DefineZynthConfigOptions["babel"] | undefined) {
  const targets = {
