...
public class ZynthSafeAreaModule: NSObject {
  ...
  @objc public static func initialize(with runtime: ZynthRuntime) -> ZynthSafeAreaModule {
    ...
    print("[ZynthSafeArea] Registering bridge")
  ...
  private func registerBridge() {
    ...
    let bridge = ZynthSafeAreaBridge(module: self)
    runtime.installModules([bridge])
    print("[ZynthSafeArea] Bridge registered with runtime")
  }
  ...
  private func publishMetricsToJS(_ metrics: WindowMetrics) {
    ...
    runtime.emitEvent(name: "ZynthSafeArea:change", payload: metrics.toDictionary())
  }
}
