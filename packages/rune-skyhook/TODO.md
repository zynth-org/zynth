...
## 1. Database & Schema
- [x] **Review Schema:** Check `packages/rune-skyhook/src/db/schema` to ensure we can store all necessary metadata.
...
- [x] **Template Source:** Uses `packages/rune-templates/app`.
...
- [x] **Integration Strategy:** Artifacts mounted to `/opt/rune-artifacts` (read-only) and `package.json` updated to use `file:` protocol.

## 5. Docker Provisioning
- [x] **Update `provisionDockerSandbox`:**
    - [x] Accepts `artifactsPath`.
    - [x] Mounts `artifactsPath` to `/opt/rune-artifacts`.
