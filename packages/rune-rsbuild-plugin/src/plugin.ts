...
import type { RuneRsbuildPluginOptions } from "./types.js";
...
export interface InternalPluginOptions extends RuneRsbuildPluginOptions {
  isWeb: boolean;
}
...
export function createRuneRsbuildPlugin(
  ...
  return {
    ...
    async setup(api) {
      ...
      const discoveredAliases = await discoverRunePackageAliases(
        repoRoot,
        isWeb
      );

      // Special handling for @rune/core in Web
      if (isWeb) {
        const coreWebEntry = await pickFirstExisting([
          path.join(repoRoot, "packages/rune-core/src/index.web.ts"),
          path.join(repoRoot, "packages/rune-core/src/index.web.tsx"),
        ]);
        if (coreWebEntry) {
          discoveredAliases["@rune/core$"] = coreWebEntry;
          discoveredAliases["@rune/core"] = path.join(repoRoot, "packages/rune-core/src");
        }
      }
      ...
      if (api.context.action === "dev") {
        api.modifyRsbuildConfig((config) => {
          ...
          defines["globalThis.__RUNE_DEV_SERVER_URL"] = JSON.stringify(devServerUrl);
