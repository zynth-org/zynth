...
Skyhook is the backend orchestration server for the Zynth framework, responsible for managing the lifecycle of Zynth applications. It handles operations such as creating new applications, updating existing ones, and integrating with an AI coding agent (Goose) to generate or modify application source code.

## Key Functionality

### 1. App Management Endpoints

Skyhook exposes the following primary API endpoints for managing Zynth applications:

-   **`POST /app/create`**:
    *   **Purpose**: Initiates the creation of a new Zynth application.
    *   **Workflow**:
...
-   **`POST /app/:appId/source/update`**:
    *   **Purpose**: Updates the source code of an existing Zynth application based on a new prompt.
    *   **Workflow**:
        ...
        6.  Re-links shared Zynth artifacts.
...
-   **Apps (formerly Projects)**: Represents a Zynth application. Stored in the database with metadata like name, description, and settings.
...
-   **Prebundled Artifacts**: A pre-install step collects JS `dist/` outputs from eligible packages into `packages/zynth-skyhook/vendor/prebundle`. Skyhook mounts this directory into the Docker sandbox so agent workspaces can use `file:` dependencies without accessing the monorepo.
...
-   Run `node scripts/build-skyhook-artifacts.ts` to populate `packages/zynth-skyhook/vendor/prebundle`.
...
-   **`apps`**: Stores metadata for each Zynth application.
    -   `id` (UUID, PK)
    -   `userId` (text)
    -   `name` (text)
    -   `description` (text)
