...
const MODULE_NAME = "RuneFileSystem";
...
function emitDevtoolsEvent(event: {
  ...
  const globalObj = getGlobalObject() as { __RUNE_DEVTOOLS__?: DevtoolsBridge };
  const bridge = globalObj.__RUNE_DEVTOOLS__;
  if (bridge?.emit) {
    bridge.emit(event);
  }
}
...
export function warnMissingNativeOnce(context: string, error?: unknown): void {
  ...
  warnedMissing = true;
  console.warn(
    `[RuneFileSystem] Native module missing (${context}). FileSystem APIs will be unavailable.`,
    error ?? ""
  );
  emitDevtoolsEvent({
