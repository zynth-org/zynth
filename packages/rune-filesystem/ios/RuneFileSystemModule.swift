//
//  RuneFileSystemModule.swift
//  RuneFileSystem
//
//  Native file system module for Rune
//
...
@objc(RuneFileSystemModule)
final class RuneFileSystemModule: NSObject, RuneModule, RuneSyncModule {
  let name: String = "RuneFileSystem"
  ...
  private func listDirectory(_ uri: String) throws -> [[String: Any]] {
    guard let path = resolvePath(uri) else {
      throw NSError(domain: "RuneFileSystem", code: 1, userInfo: [NSLocalizedDescriptionKey: "Invalid uri"])
    }
  ...
  private func createDirectory(_ uri: String, options: [String: Any]?) throws {
    ...
    if fileManager.fileExists(atPath: path) {
      if overwrite {
        ...
        throw NSError(domain: "RuneFileSystem", code: 2, userInfo: [NSLocalizedDescriptionKey: "Destination already exists"])
      }
    }
  ...
  private func deleteItem(_ uri: String, recursive: Bool) throws {
    ...
    if isDirectory.boolValue && !recursive {
      ...
      if !contents.isEmpty {
        throw NSError(domain: "RuneFileSystem", code: 3, userInfo: [NSLocalizedDescriptionKey: "Directory not empty"])
      }
    }
    try fileManager.removeItem(atPath: path)
  }
  ...
  private func copyItem(_ from: String, _ to: String) throws {
    ...
    if fileManager.fileExists(atPath: toPath) {
      throw NSError(domain: "RuneFileSystem", code: 4, userInfo: [NSLocalizedDescriptionKey: "Destination already exists"])
    }
  ...
  private func writeBase64(_ uri: String, base64: String) throws {
    ...
    guard let data = Data(base64Encoded: base64) else {
      throw NSError(domain: "RuneFileSystem", code: 5, userInfo: [NSLocalizedDescriptionKey: "Invalid base64"])
    }
    try ensureParentDirectories(for: path)
