...
class RuneBottomSheetLayout @JvmOverloads constructor(
  ...
  private val dialog = RuneBottomSheetDialog(context, contentHost)
  ...
  init {
    dialog.listener = object : RuneBottomSheetDialog.Listener {
      override fun onShow() {
  ...
  fun bind(manager: RuneUIManager, node: RuneUIManager.Node) {
  ...
  fun applyLayoutOptions(options: RuneBottomSheetOptions?) {
    val resolved = options ?: RuneBottomSheetOptions()
