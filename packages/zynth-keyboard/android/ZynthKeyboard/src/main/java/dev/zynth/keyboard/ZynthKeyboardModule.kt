...
class ZynthKeyboardModule(
    ...
    private fun registerBridge() {
        val bridge = ZynthKeyboardBridge(this)
    ...
    private fun attachToRootView() {
        ...
        // other modules (like ZynthSafeArea) that attach to the decor view root.
        ...
        android.util.Log.d("ZynthKeyboard", "Attached to root view: ${rootView?.javaClass?.simpleName} (id=${rootView?.id})")
...
// Only process IME animations — return early if none present
    ...
    companion object {
        private const val EVENT_NAME = "ZynthKeyboard:change"
    }
}
