...
public class ZynthHypervisorView: UIView {
    ...
    func destroy() {
        print("[ZynthHypervisor] Destroying ZynthRuntime for Hypervisor View")
        isDestroyed = true
        destroyRuntime()
    }
    ...
    private func loadGuest() {
        ...
        // Dynamic module auto-discovery via ZynthNativeModules.json
        if let configURL = Bundle.main.url(forResource: "ZynthNativeModules", withExtension: "json"),
           ...
           let modules = try? JSONSerialization.jsonObject(with: data, options: []) as? [[String: String]] {
            ...
            for moduleConfig in modules {
                ...
                if moduleClass.responds(to: selector) {
                    print("[ZynthHypervisor] Auto-installing module: \(className)")
        ...
        print("[ZynthHypervisor] Guest runtime created with surfaceId: \(newRuntime.rootSurfaceId)")
        ...
        guard let message = notification.userInfo?["message"] else { return }
            if let dict = message as? NSDictionary {
                ...
                if let manager = manager, let node = node {
                    manager.zynth_dispatchEvent("onMessage", payload: dict as? [AnyHashable: Any], to: node)
                }
            } else {
                let payload: NSDictionary = ["data": message]
                onMessage?(payload)
                if let manager = manager, let node = node {
                    manager.zynth_dispatchEvent("onMessage", payload: payload as? [AnyHashable: Any], to: node)
                }
            }
        }
    
    func postMessage(_ message: Any) {
        runtime?.emitEvent(name: "ZynthHypervisor:Message", payload: message)
    }
    ...
    deinit {
        print("[ZynthHypervisor] Deallocating Hypervisor View")
        destroy()
    }

    private func loadBundle(from url: URL, runtime: ZynthRuntime) {
        if url.isFileURL {
            do {
    ...
    private func notifyError(_ message: String) {
        ...
        onError?(payload)
        if let manager = manager, let node = node {
            manager.zynth_dispatchEvent("onError", payload: payload as? [AnyHashable: Any], to: node)
        }
    }
    ...
    private func notifyLoad() {
        onLoad?()
        if let manager = manager, let node = node {
            manager.zynth_dispatchEvent("onLoad", payload: [:], to: node)
        }
    }
}
