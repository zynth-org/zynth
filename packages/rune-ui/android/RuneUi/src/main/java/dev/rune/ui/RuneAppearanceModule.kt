...
class RuneAppearanceModule(
    ...
    private fun registerBridge() {
        android.util.Log.d("RuneAppearance", "registerBridge")
        runtime.installModules(listOf(RuneAppearanceBridge(this)))
    }

    private fun startObserving() {
        android.util.Log.d("RuneAppearance", "startObserving")
        val observer = object : ComponentCallbacks {
            override fun onConfigurationChanged(newConfig: Configuration) {
                android.util.Log.d("RuneAppearance", "configurationChanged uiMode=${newConfig.uiMode}")
    ...
    private fun updateState(config: Configuration, force: Boolean) {
        ...
        android.util.Log.d("RuneAppearance", "updateState colorScheme=${newState.colorScheme}")
        lastState = newState
        publishState(newState)
    }

    private fun publishState(state: AppearanceState) {
        android.util.Log.d("RuneAppearance", "emit RuneAppearance:change ${state.toMap()}")
        runtime.emitEvent("RuneAppearance:change", state.toMap())
    }
