...
const MODULE_KEY = "RuneAppearance";
const EVENT_NAME = "RuneAppearance:change";

declare global {
  var NativeConstants: Record<string, unknown> | undefined;
  var __RUNE_APPEARANCE__: NativeAppearanceModule | undefined;
}
...
function readFromNativeModule(): ColorScheme | null {
  const globalObj = getGlobalObject() as {
    __RUNE_APPEARANCE__?: NativeAppearanceModule;
  };
  const module = globalObj.__RUNE_APPEARANCE__ ?? null;
...
export function subscribeToSystemColorScheme(
  ...
  const module = globalObj.__RUNE_APPEARANCE__;
