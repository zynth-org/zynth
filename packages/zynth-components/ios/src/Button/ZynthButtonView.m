...
@property(nonatomic, assign) BOOL zynthLoading;
...
@implementation ZynthButtonView

- (BOOL)zynth_shouldUseGlassConfiguration {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 260000
  if (@available(iOS 26.0, *)) {
    return self.zynthGlassEnabled;
  }
#endif
  return NO;
}
...
- (void)updateNativeConfiguration {
  ...
  // Role
  if ([self.zynthRole isEqualToString:@"destructive"]) {
    self.role = UIButtonRoleDestructive;
    self.tintColor = [UIColor systemRedColor];
  } else if ([self.zynthRole isEqualToString:@"cancel"]) {
  ...
  // Disabled State
  self.enabled = !self.zynthDisabled;
...
- (void)zynth_setRole:(NSString *)role {
  _zynthRole = role ?: @"normal";
  [self updateNativeConfiguration];
}
...
- (void)zynth_setTitle:(NSString *)title {
  _buttonTitle = title;
  [self updateNativeConfiguration];
}

- (void)zynth_setBaseColor:(UIColor *)color {
  _baseColor = color;
  [self updateNativeConfiguration];
}

- (void)zynth_setImage:(UIImage *)image {
  _buttonImage = image;
  [self updateNativeConfiguration];
}
...
- (void)zynth_setMinimumTouchSize:(id)sizeValue {
...
- (void)zynth_setHapticsMode:(NSString *)mode {
...
- (void)startLongPressTimer {
  [_longPressTimer invalidate];
  _longPressTimer = [NSTimer scheduledTimerWithTimeInterval:kZynthButtonLongPressDuration
                                                     target:self
...
- (void)handleLongPressTimer {
  ...
  if (self.delegate && [self.delegate respondsToSelector:@selector(buttonView:didLongPressWithDuration:)]) {
    [self.delegate buttonView:self didLongPressWithDuration:kZynthButtonLongPressDuration * 1000.0];
  }
}
