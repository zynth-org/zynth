...
enum ZynthModalAnimation: String {
  case fade
  case slide
  case zoom
  case none

  static func from(_ value: String?) -> ZynthModalAnimation {
    guard let value else { return .fade }
    return ZynthModalAnimation(rawValue: value.lowercased()) ?? .fade
  }
}

struct ZynthModalOptions {
  var animation: ZynthModalAnimation = .fade
...
final class ZynthModalPresenter: NSObject {
  private weak var host: ZynthModalView?
  private let contentHost: UIView
  private var options = ZynthModalOptions()
  private var contentController: ZynthModalContentViewController?
  private var isOpen = false

  init(contentHost: UIView, host: ZynthModalView) {
    self.contentHost = contentHost
    self.host = host
    super.init()
  }

  func updateOptions(_ newOptions: ZynthModalOptions) {
    options = newOptions
  ...
  private func presentInternal() {
    ...
    let controller = ZynthModalContentViewController(
...
extension ZynthModalPresenter: UIViewControllerTransitioningDelegate {
  func animationController(
    forPresented presented: UIViewController,
    presenting: UIViewController,
    source: UIViewController
  ) -> UIViewControllerAnimatedTransitioning? {
    return ZynthModalAnimator(isPresenting: true, animation: options.animation)
  }

  func animationController(
    forDismissed dismissed: UIViewController
  ) -> UIViewControllerAnimatedTransitioning? {
    return ZynthModalAnimator(isPresenting: false, animation: options.animation)
  }
}

extension ZynthModalPresenter: UIAdaptivePresentationControllerDelegate {
  func presentationControllerDidDismiss(_ presentationController: UIPresentationController) {
    completeDismiss()
  }
}

private final class ZynthModalAnimator: NSObject, UIViewControllerAnimatedTransitioning {
  private let isPresenting: Bool
  private let animation: ZynthModalAnimation

  init(isPresenting: Bool, animation: ZynthModalAnimation) {
  ...
  func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
    ...
    let modalController = (isPresenting
      ...
      : transitionContext.viewController(forKey: .from)) as? ZynthModalContentViewController
