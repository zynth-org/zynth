    ...
    <div style="display: contents">
      ...
      <div
        style={{
          ...
          padding: "4px 0",
          overflow: "hidden",
          animation: isOpen() ? "zynth-menu-fade 0.15s ease-out" : "none",
        }}
        onClick={(e) => e.stopPropagation()}
      >
        <span data-zynth-slot="items" style="display: contents" />
      </div>

      <style>
        {`@keyframes zynth-menu-fade { from { opacity: 0; transform: translateY(-4px); } to { opacity: 1; transform: translateY(0); } }`}
      </style>
    </div>
  );
};
...
export const MenuItem = (props: any) => {
  ...
  const handleClick = (e: MouseEvent) => {
    ...
    e.target?.dispatchEvent(
      new CustomEvent("zynth-menu-close", { bubbles: true })
    );
  };
