...
const SCROLLBAR_STYLE_ID = "zynth-scrollbar-style";
...
const ensureScrollbarStyle = () => {
  ...
  style.textContent = `
    .zynth-scroll-view-hide-scrollbar::-webkit-scrollbar { display: none; }
  `;
  document.head.appendChild(style);
};

const getState = (element: HTMLElement): ScrollState => {
  const existing = (element as any).__zynth_scroll_state as
    | ScrollState
    | undefined;
  ...
  (element as any).__zynth_scroll_state = next;
  return next;
};
...
registerWebComponent("scroll-view", {
  create: (initialProps) => {
    ...
    element.classList.add("zynth-scroll-view");
  ...
  updateProp: (element, key, value) => {
    ...
    if (
      ...
      const showsVertical =
        key === "showsVerticalScrollIndicator"
          ? value !== false
          : (element as any).__zynth_shows_vertical ?? true;
      const showsHorizontal =
        key === "showsHorizontalScrollIndicator"
          ? value !== false
          : (element as any).__zynth_shows_horizontal ?? true;

      if (key === "showsVerticalScrollIndicator") {
        (element as any).__zynth_shows_vertical = value !== false;
      }
      if (key === "showsHorizontalScrollIndicator") {
        (element as any).__zynth_shows_horizontal = value !== false;
      }

      if (!showsVertical || !showsHorizontal) {
        ensureScrollbarStyle();
        element.classList.add("zynth-scroll-view-hide-scrollbar");
        (element.style as any).scrollbarWidth = "none";
        (element.style as any).msOverflowStyle = "none";
      } else {
        element.classList.remove("zynth-scroll-view-hide-scrollbar");
