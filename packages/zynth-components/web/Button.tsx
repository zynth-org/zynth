...
export const Button = (props: ButtonProps) => {
  ...
  const renderSpinner = () => (
    <div
      class="zynth-button-loading-container"
      style={{
        display: "inline-flex",
        "align-items": "center",
  ...
  return (
    <button
      ...
      class={`zynth-button${local.class ? ` ${local.class}` : ""}`}
      {...rest}
    >
      <Show
      ...
      <style>
        {`
          .zynth-button:active:not(:disabled) { opacity: 0.75 !important; transform: scale(0.98); }
        `}
      </style>
    </button>
  );
