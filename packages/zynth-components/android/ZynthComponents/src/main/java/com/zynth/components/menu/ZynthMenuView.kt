...
class ZynthMenuView(context: Context) : FrameLayout(context) {
  var manager: ZynthUIManager? = null
  var nodeId: Int = -1
  var hasOnOpenHandler = false
  var hasOnCloseHandler = false

  private var triggerView: ZynthMenuTriggerView? = null
  ...
  override fun onViewAdded(child: View) {
    ...
    if (child is ZynthMenuTriggerView) {
      attachTrigger(child)
    }
  }
  ...
  private fun attachTrigger(trigger: ZynthMenuTriggerView) {
  ...
  fun showMenuFromTrigger(anchor: View) {
    ...
    val itemMap = LinkedHashMap<Int, ZynthMenuItemView>()
  ...
  private fun collectMenuItems(): List<ZynthMenuItemView> {
    val items = ArrayList<ZynthMenuItemView>()
    for (i in 0 until childCount) {
      val child = getChildAt(i)
      if (child is ZynthMenuItemView) {
        items.add(child)
      }
    }
    return items
