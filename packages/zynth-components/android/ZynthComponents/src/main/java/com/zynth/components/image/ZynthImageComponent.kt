...
/**
 * ZynthImageComponent handles image loading, measurement, and lifecycle for Image nodes.
...
internal class ZynthImageComponent(
  ...
  fun handleProp(node: ZynthUIManager.Node, name: String, jsonValue: String?): Boolean {
  ...
  fun onHandlerSet(node: ZynthUIManager.Node, event: String) {
    val state = ensureState(node)
    when (event) {
  ...
  private fun applyImageSource(node: ZynthUIManager.Node, rawValue: Any?) {
  ...
  private fun clearImage(node: ZynthUIManager.Node) {
  ...
  private fun loadWithCoil(node: ZynthUIManager.Node, data: Any?, token: String, scale: Float = 1f) {
    val state = ensureState(node)
  ...
  private fun loadInlineData(node: ZynthUIManager.Node, spec: ImageSourceSpec.InlineData, token: String) {
  ...
  private fun loadSystemImage(node: ZynthUIManager.Node, spec: ImageSourceSpec.SystemData, token: String) {
  ...
  private fun loadAssetImage(node: ZynthUIManager.Node, spec: ImageSourceSpec.AssetData, token: String) {
  ...
  private fun loadRemoteImage(node: ZynthUIManager.Node, spec: ImageSourceSpec.RemoteData, token: String) {
  ...
  private fun dispatchImageEvent(node: ZynthUIManager.Node, event: String, payload: JSONObject) {
  ...
  private fun applyTintColor(node: ZynthUIManager.Node, imageView: ImageView, value: Any?) {
    val state = ensureState(node)
