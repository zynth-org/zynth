...
class DatePickerComponentRegistrar : ZynthComponentRegistrar {
  override fun register(registry: ZynthComponentRegistry) {
    ...
    Log.d("ZynthComponents", "Registered DatePicker component")
  }
}

private fun createDatePickerViewDescriptor(): ZynthComponentDescriptor {
  return ZynthComponentDescriptor(
    type = "date-picker-view",
    createView = { context: Context, _ ->
      ZynthDatePickerView(context).apply {
    ...
    onNodeCreated = { manager: ZynthUIManager, node: ZynthUIManager.Node ->
      (node.view as? ZynthDatePickerView)?.let { pickerView ->
    ...
    applyProperty = { node, name, value ->
      val pickerView = node.view as? ZynthDatePickerView ?: return@ZynthComponentDescriptor false
      when (name) {
        "mode" -> {
    ...
    onReset = { node ->
      (node.view as? ZynthDatePickerView)?.reset()
    },
  )
}

private fun createDatePickerTriggerDescriptor(): ZynthComponentDescriptor {
  return ZynthComponentDescriptor(
    type = "date-picker-trigger-view",
    createView = { context: Context, _ ->
      ZynthDatePickerTriggerView(context).apply {
        layoutParams = layoutParams ?: FrameLayout.LayoutParams(
          FrameLayout.LayoutParams.WRAP_CONTENT,
          FrameLayout.LayoutParams.WRAP_CONTENT,
        )
    ...
    onReset = { node ->
      (node.view as? ZynthDatePickerTriggerView)?.reset()
    },
  )
}
