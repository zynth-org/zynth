...
function normalizeTapEvent(raw: unknown): TapGestureEvent {
  ...
  return {
    kind: "tap",
    ...base,
    numberOfTaps: readNumber(value, "numberOfTaps", 1),
  };
}
...
function invokePhaseCallback<TEvent extends BaseGestureEvent>(
  ...
  if (event.phase === "start") {
    invoke(definition.onStart, event);
    return;
  }
  if (event.phase === "update") {
    invoke(definition.onUpdate, event);
    return;
  }
  if (event.phase === "end") {
    invoke(definition.onEnd, event);
    return;
  }
...
export const GestureDetector: ParentComponent<GestureDetectorProps> = (props) => {
  ...
  const callWithOwner = <TEvent extends BaseGestureEvent>(
    ...
    if (!callback) return;
    if (owner) {
      runWithOwner(owner, () => callback(event));
      return;
    }
    callback(event);
  };
  ...
  const tapGestures = createMemo(() =>
    gestures().filter((gesture) => gesture.kind === "tap") as TapGesture[]
  );
  const longPressGestures = createMemo(() =>
    gestures().filter((gesture) => gesture.kind === "longPress") as LongPressGesture[]
  );
  const rotationGestures = createMemo(() =>
    gestures().filter((gesture) => gesture.kind === "rotation") as RotationGesture[]
  );
  const pinchGestures = createMemo(() =>
    gestures().filter((gesture) => gesture.kind === "pinch") as PinchGesture[]
  );
  const flingGestures = createMemo(() =>
    gestures().filter((gesture) => gesture.kind === "fling") as FlingGesture[]
  );
  const panGestures = createMemo(() =>
    gestures().filter((gesture) => gesture.kind === "pan") as PanGesture[]
  );
  ...
  const handleRotationGesture = (event: unknown) => {
    ...
    for (const gesture of rotationGestures()) {
      invokePhaseCallback(gesture, normalized, callWithOwner);
    }
  };
  ...
  const handleFlingGesture = (event: unknown) => {
    ...
    for (const gesture of flingGestures()) {
      invokePhaseCallback(gesture, normalized, callWithOwner);
    }
  };
  ...
  const handlePanGesture = (event: unknown) => {
    ...
    for (const gesture of panGestures()) {
      invokePhaseCallback(gesture, normalized, callWithOwner);
    }
  };
