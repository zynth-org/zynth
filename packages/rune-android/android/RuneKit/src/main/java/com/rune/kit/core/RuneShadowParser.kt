...
private const val DEFAULT_ALPHA = 0x40 // ~25% opacity
...
/**
 * Parser for CSS-like `box-shadow` values plus React Native shadow props.
 * Normalizes into ShadowLayer structs so Android/iOS renderers can share semantics.
 */
object RuneShadowParser {
  // Matches hex, named colors, and functional color syntaxes (rgb/rgba/hsl/hsla/hwb)
  ...
  private fun parseString(raw: String): ShadowLayer? {
    ...
    // Extract explicit color token (supports rgba()/hsl() etc. with spaces)
    val colorMatch = COLOR_TOKEN.find(working)
    var color: Int? = null
    if (colorMatch != null) {
      color = RuneColorParser.parse(colorMatch.value)
    ...
    for (token in tokens) {
      ...
      if (color == null) {
        color = RuneColorParser.parse(token)
      }
    }
