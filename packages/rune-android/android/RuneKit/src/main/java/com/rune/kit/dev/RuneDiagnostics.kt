...
/**
 * Unified diagnostics reporting system for Rune.
 * Similar to iOS RuneDiagnostics.swift.
 */
object RuneDiagnostics {
    private const val TAG = "RuneDiagnostics"
    
    @Volatile
    private var currentRedBox: RuneRedBoxView? = null
    ...
    private fun showRedBox(activity: Activity, message: String, stack: String?) {
        ...
        val redBox = RuneRedBoxView(activity).apply {
 ...
 */
@Suppress("unused")
fun runeDiagnosticsReportJNI(phase: String, message: String, stack: String?) {
    RuneDiagnostics.report(phase, message, stack)
}