...
final class ZynthDimensionsModule: NSObject, ZynthModule, ZynthSyncModule {
  ...
  private func computePayload() -> DimensionsPayload {
    ...
    if let root = rootView, !root.bounds.isNull, root.bounds.width > 0, root.bounds.height > 0 {
      // print("[ZynthDimensions] Using rootView bounds: \(root.bounds)")
      windowBounds = root.bounds
    } else if let bounds = bestWindowBounds() {
      // print("[ZynthDimensions] Using bestWindowBounds: \(bounds)")
      windowBounds = bounds
    } else {
      // print("[ZynthDimensions] Fallback to screenBounds: \(screenBounds)")
  ...
  private func bestWindowBounds() -> CGRect? {
    if #available(iOS 13.0, *) {
      ...
      if let bounds = keyWindow?.bounds, !bounds.isNull, bounds.width > 0, bounds.height > 0 {
        return bounds
      }
    }
    let delegateWindow = UIApplication.shared.delegate?.window ?? nil
    if let bounds = delegateWindow?.bounds, bounds.width > 0, bounds.height > 0 {
      return bounds
    }
    let keyWindow = UIApplication.shared.keyWindow
    if let bounds = keyWindow?.bounds, bounds.width > 0, bounds.height > 0 {
      return bounds
    }
    return nil
  }
